...
least-evidenced of the seventeen mandated domains (81 blobs, 3.0% of corpus)**. This register
...
| BN-05 | Dropship sale | Order → Purchase to vendor → Vendor ships to customer → AP + AR recognition → Accounting | **No** own-warehouse movement | Yes | **`HOLD`** | **RESTATED BY CORR1 — see `SA20` §2.2.** Dropship *is* evidenced: 66 unique paths across the remote, and the Group A purchase capability model (`8b0993d8`) records it as a capability with status `VERIFIED FACT`, states its mechanism, gives the received-quantity netting rule, and carries a database-confirmed link from a purchase line to a sales line flagged for a future cross-module phase. `HOLD` is retained for a **different and narrower** reason: whether SMEsPlus records a non-physical inventory event for title passage remains undetermined, and that is an accounting-material design question. |
