...
| `ACC-WHT-06` | Multiple WHT types/rates in one document (Boss Challenge Addendum, added mid-session) — see `08_ACC_WHT_MULTI_TYPE_MULTI_RATE_CHALLENGE.md` | **MIXED: HIGH GAP (base module payment-posting) / RESOLVED (with `l10n_th_withholding_tax_multi` installed) / PARTIAL (certificate income-type-per-line)** | WHT is modeled at the **line** level (never PO-level — zero `purchase.order` references found), and a single bill can freely carry multiple distinct `wt_tax_id` values across lines (no cross-line constraint). At **payment** time, the base module (`l10n_th_withholding_tax` alone) only correctly routes a WHT write-off to a GL account when `len(wt_tax) == 1` (`wizard/account_payment_register.py:64`) — with 2+ distinct rates on one payment, the amount still nets correctly but **no line is tagged with a WHT account or `wt_tax_id`**, breaking certificate and PND traceability for the entire payment. This is confirmed a **HIGH GAP** per Boss's own instruction. With `l10n_th_withholding_tax_multi` also installed, each rate gets its own correctly-tagged `account.payment.deduction` line (`l10n_th_withholding_tax_multi/models/account_payment.py:37-92`) and per-rate partial-payment proration — **RESOLVED** in that configuration. The 50-twi certificate correctly captures multiple rate lines with independent amounts (one certificate, multiple `wt_line` rows), but the statutory income-type classification (`wt_cert_income_type`) is a single wizard-selected value stamped uniformly across every line — the schema supports per-line differentiation but the create-flow code does not use it (`withholding_tax_cert.py:284-307`, `create_withholding_tax_cert.py:88-188`) — **PARTIAL/GAP**. The PND3/PND53 export has no grouping at all (one row per journal line), so once lines are correctly tagged, multiple rates correctly appear as separate, correctly-typed rows — but this is conditional on the payment-posting gap above being resolved first. **Open question this audit cannot resolve from source alone: whether `l10n_th_withholding_tax_multi` is part of the intended reference/target module combination** — this materially changes the disposition and must be a Boss/Accounting-Tax decision, not an audit assumption. |
...
`ACC-WHT-06` (Boss Challenge Addendum, multiple WHT types/rates in one document) has been answered in full — see `08_ACC_WHT_MULTI_TYPE_MULTI_RATE_CHALLENGE.md` and §2/§6/§8 above — per Boss's instruction not to close this session until that item was addressed. It is answered, not closed: it surfaced a genuine HIGH GAP (base-module-only multi-rate payment posting) whose resolution depends on a module-baseline decision only Boss/Accounting-Tax can make, and it is carried forward accordingly under Option B.
