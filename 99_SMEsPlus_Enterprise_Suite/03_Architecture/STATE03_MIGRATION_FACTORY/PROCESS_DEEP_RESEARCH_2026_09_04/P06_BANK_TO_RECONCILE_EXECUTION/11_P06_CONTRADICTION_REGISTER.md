...
| **T-10** | P10 routes its close and FX dependencies to **P04**; P11 and P02 route the same questions to **P08** | **NOT RESOLVED — not P06's to adjudicate.** Flagged to P11 as a routing defect. **CORRECTED **[REV-E-20, 2026-09-06]**: the superseded wording read *"no P08 branch exists to answer"*. **P08 IS PUBLISHED** — `research/account-p08-record-to-report-2026-09-04-001`, 39 files, read and quoted by line in `53_`. The routing divergence stands; its consequence is worse — the dependencies are addressed to a process that exists, is published, and was not asked.** `OQ-93`. |
