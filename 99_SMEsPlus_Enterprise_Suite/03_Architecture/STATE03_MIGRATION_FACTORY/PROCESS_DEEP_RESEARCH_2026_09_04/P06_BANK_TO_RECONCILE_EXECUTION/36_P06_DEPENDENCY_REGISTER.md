...
~~The eighth settlement path~~ → **The settlement path `SR-04`** [REV-E-23, 2026-09-06 — stale current claim; found by independent frozen-surface verification, invisible to the pattern declared for its class] (cash moving with no payment object) was found by the expense process, **which had already counted it as path 5 of its own 7** (`55_` SDD-F-01; *"eighth"* withdrawn at `REV-E-12`). **A denominator P06 published as complete was incomplete, and the correction came from outside.** This is the second time in this package that an independent pass corrected a P06 denominator — the first was the ingestion-door count (REV-E-01).
