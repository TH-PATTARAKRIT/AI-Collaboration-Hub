...
- **This does not withdraw any of them.** Each was already declared with its path set, and this build is the correct target. It sharpens what "tree scope" means and is recorded as **`P06-B-55` — every tree-scope negative inherits the boundary of the *loadable* addon set.** ~~"the evidence base is a filtered distribution"~~ → **it is a RELOCATION: 791 loadable of 1752, with 961 directories in `addons_archive` excluded by the project's own `odoo.conf` and since searched** **[REV-E-21, 2026-09-06 — correction recorded in `62_` but never edited into this register]** (`REV-E-16`, `56_`).
