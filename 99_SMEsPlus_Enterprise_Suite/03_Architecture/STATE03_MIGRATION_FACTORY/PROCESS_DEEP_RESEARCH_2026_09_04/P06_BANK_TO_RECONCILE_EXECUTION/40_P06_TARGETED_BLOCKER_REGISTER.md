...
| `P06-B-53` | ~~**An eighth settlement door**~~ → **A settlement path already counted by P05 as path 5 of its own 7** *("eighth door" withdrawn — `REV-E-12`, `55_` SDD-F-01/02;**[REV-E-22, 2026-09-06 — stale current claim missed by the round-5 repair pass]**)*: the advance cash-return path moves cash through a bank journal with **no payment object**, invisible to the matching model. **INHERITED FROM P05 `SR-04` — not a P06 discovery. Flagged for deduplication at P11** so it is not counted twice across registers. | **HOLD — PEER PROCESS REQUIRED (P05)** |
| `P06-B-54` | ~~P01 and P08 packages unpublished~~ → **BOTH ARE PUBLISHED** **[REV-E-22, 2026-09-06 — stale publication status; all ten peer branches P01–P10 exist on origin, verified by `git ls-remote --heads origin`. NOT CONSUMED — `P06-OQ-124`/`P06-OQ-128`]**; P08 read (`53_`), **P01 unconsumed** (`P06-OQ-124`). Vendor-payable ownership and the close architecture still cannot be reconciled — they need a peer **ruling**, not a peer **package** | **HOLD — PEER PROCESS REQUIRED** |
...
**Boss / governance (5)**
`B-01` no receiving process specification — **HOLD — DESIGN DECISION REQUIRED** · `B-02` no Jira work item — **HOLD — DESIGN DECISION REQUIRED** · `B-03` **CLOSED — CROSS-PROCESS EVIDENCE VERIFIED** · `B-42` deliverable-list gap — **HOLD — DESIGN DECISION REQUIRED** · `B-54` ~~P01/P08 unpublished~~ → both published; P08 read, P01 unconsumed **[REV-E-22, 2026-09-06 — stale publication status; all ten peer branches P01–P10 exist on origin, verified by `git ls-remote --heads origin`. NOT CONSUMED — `P06-OQ-124`/`P06-OQ-128`]** — **HOLD — PEER PROCESS REQUIRED**
...
**State & settlement (11)**
`B-04` — **HOLD — DESIGN DECISION REQUIRED** (owner now named: P05 operates the door, P06 owns it) · `B-05` — **HOLD — DESIGN DECISION REQUIRED** · `B-06` no bank-confirmation fact — **HOLD — DESIGN DECISION REQUIRED**, semantics now specified in `25_` · `B-07` — **HOLD — DESIGN DECISION REQUIRED** · `B-16` internal transfer unpaired — **HOLD — DESIGN DECISION REQUIRED** · `B-30` accounting outside the webhook — **HOLD — DESIGN DECISION REQUIRED** · `B-31` unverified-input order cancel (v14 copy) — **HOLD — DATABASE EVIDENCE REQUIRED** (deployment unknown) · `B-33` no cash session — **HOLD — DESIGN DECISION REQUIRED** · `B-34` PDC not migrated — **HOLD — DESIGN DECISION REQUIRED** (re-verified: no PDC concept in v18 reference at all) · `B-35` returned payment not migrated — **HOLD — DESIGN DECISION REQUIRED** (re-verified: no `payment.return` model, no chargeback) · `B-53` ~~eighth settlement door~~ → settlement path already counted by P05 as its path 5 **[REV-E-22, 2026-09-06]** — **HOLD — PEER PROCESS REQUIRED**
...
| `P06-B-64` | **Correction populations were scoped by phrase, not by claim class.** Round 5 corrected **31 of 53** current statements (58%) and reported 100%. Every one of the 22 misses follows from grepping the peer's *name* or the remembered *phrase* instead of the *claim*. **The independent verification round repeated the same error before catching it** | **HIGH** | **HOLD — INDEPENDENT AUDIT REQUIRED** (`AASP-VETO-07`). `FACT VERIFIED` — `P06_CORRECTION_PROPAGATION_MATRIX.md` `PROP-F-01`…`F-03` |
| `P06-B-65` | **An AAS+ veto condition stood on a false premise for two rounds.** `66_`:16 read *"(c′) P01 read — NOT MET, still unpublished"*; **P01 has been published at `b820b29` since 2026-09-04**. The condition is still unmet, but **by scope, not by availability** — and those license different Boss decisions | **HIGH** | **CORRECTED `REV-E-22`; the condition remains NOT MET.** `FACT VERIFIED` — `git ls-remote --heads origin` |
...
| Unit | Prior | **Now** |
|---|---|---|
| Blockers `P06-B-*` | 58 | **65** *(63 at G02 close; `B-64`, `B-65` added by the independent correction-integrity verification, `774aa0b` round)* |
