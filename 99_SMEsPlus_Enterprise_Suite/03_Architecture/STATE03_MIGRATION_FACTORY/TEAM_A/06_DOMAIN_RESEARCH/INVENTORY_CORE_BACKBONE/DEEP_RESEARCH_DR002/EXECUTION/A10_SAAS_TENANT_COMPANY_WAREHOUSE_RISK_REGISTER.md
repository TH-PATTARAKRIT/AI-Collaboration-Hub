...
| SAAS-02 | "Branch" in this codebase's Thai-localization layer has a **structurally available** child-`res.company` implementation path (`res_company.parent_id`/`parent_path`, not confirmed as this customer's actual practice — single-company dataset, corrected per CORR-005/IER-003 §06 below), structurally unconnected to warehouse/location; a separate "Thai Tax Branch" `Char` field on `res.partner` has **zero** structural link to that company hierarchy | GROUP A Scenario 11 finding (frozen evidence, reused DELTA-FIRST) + IER-003 independent correction | source code direct + independent review corroboration | High | If SMEsPlus intends real branch-aware inventory (a named Thailand requirement per the Roadmap's Thai-branch context note), the reference source offers **two independent, uncoordinated** implementations of "branch" — neither is a ready-made pattern to adopt as-is |
...
## CORR-005 disposition note (2026-09-01)

Per the Boss Inventory Scope Ruling (`BOSS_INVENTORY_SCOPE_RULING_BH_EXCLUSION_AND_BRANCH_BASELINE_2026_09_01.md`), SAAS-02 (= GRPA-H8) is `CLOSED AS AN INVENTORY ARCHITECTURE QUESTION` — SMEsPlus's approved Tenant/Company/Branch platform baseline is not reopened by this register, and Inventory does not own the Branch model definition. This row remains a valid **risk observation about the reference source** (unchanged purpose of this register); it is no longer an open Inventory research blocker. See [A14](A14_UNKNOWN_CONFLICT_EVIDENCE_GAP_REGISTER.md) §Part 3 for the full reconciliation.
