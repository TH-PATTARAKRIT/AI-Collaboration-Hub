...
- GROUP A's Scenario 11 finding (E2E Lifecycle Map, independently unrebutted): a completely separate "Thai Tax Branch" `Char` field exists on `res.partner` with **zero structural connection** to the company hierarchy. This is a material SaaS/multi-entity finding, carried into A10.
...
> **CORR-005 correction (2026-09-01), superseding this section's original "Branch... is a child `res.company` record" wording**: Independent Review IER-003 ([06_IER003_HIGH_H3_THAI_BRANCH_TBRAC_REVIEW.md](../../../../../INDEPENDENT_REVIEW/INVENTORY_CORE_BACKBONE/IER_003/EXECUTION/06_IER003_HIGH_H3_THAI_BRANCH_TBRAC_REVIEW.md)) found that `res_company.parent_id`/`parent_path` (standard Odoo multi-company hierarchy columns) exist in the schema — the structural *capability* for a child-company branch pattern is real — but this customer's database contains exactly **one** `res.company` row with `parent_id` unset. There is no second, child-company record to examine. The corrected statement is: **a child-`res.company` branch pattern is structurally available in this codebase, but is not confirmed as this customer's actual practice** (single-company dataset). The two-Char-field conflict described above (the separate, uncoordinated Thai Tax Branch field) is unaffected by this correction and remains fully confirmed. Per the Boss Inventory Scope Ruling (`BOSS_INVENTORY_SCOPE_RULING_BH_EXCLUSION_AND_BRANCH_BASELINE_2026_09_01.md`), this finding (GRPA-H8/SAAS-02) is `CLOSED AS AN INVENTORY ARCHITECTURE QUESTION`; the residual legacy `branch` vs. `company_registry` mapping question is a `CONTROLLED MIGRATION / TBRAC CARRY-FORWARD`, not an open Inventory research blocker — see [A14](A14_UNKNOWN_CONFLICT_EVIDENCE_GAP_REGISTER.md) §Part 3 and [A12](A12_MIGRATION_PROVENANCE_AND_CONTINUITY_EVIDENCE.md) §12.
