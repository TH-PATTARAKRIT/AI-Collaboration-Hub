...
echo "| Check | Result |"
echo "|---|---|"
R="/Volumes/iMacSys/SMEsPlus ENTERPRISE SUITE/ACCOUNT_WAVE_A_MCC_2026_09_04_EXECUTION"
DIRTY=$(git -C "$R" status --porcelain | grep -v 'METHOD_CONVERGENCE_CLOSURE' | wc -l | tr -d ' ')
ANC=$(git -C "$R" merge-base --is-ancestor 33cdc6fa009c4eafcca543c253ccad19e97fd0dc HEAD && echo yes || echo no)
BR=$(git -C "$R" branch --show-current)
echo "| Branch | \`$BR\` |"
echo "| Tracked files modified outside the MCC package | **$DIRTY** |"
echo "| Parent commit \`33cdc6fa009c4eafcca543c253ccad19e97fd0dc\` reachable from HEAD | **$ANC** |"
} > "$OUT"
echo "written: $OUT"
