...
## 12. ADDENDUM — `ER-CORE-3` re-specified as a mechanism, after it failed in this round's own hands

The correction-propagation rule at §4 was written by this round and **this round then failed it**, on
its own last correction, hours after writing it (`MCC_J` `J-16`). The audit panel found the package
publishing two closure counts, two case floors, and two contradictory dispositions of one id.

> **An authoring rule cannot fix a propagation defect, for the same reason an authoring rule cannot fix
> an unbounded negative: the person who must apply it is the person whose error it is.**
> This is `ER-CORE-5` — the discovery-asymmetry rule — applied to corrections rather than to findings,
> and it is the same finding one level up.

### `ER-CORE-3` clause 6 — the governing-register mechanism · `DOMAIN-INDEPENDENT`

> **Every package publishes ONE canonical figures register. Every figure and every disposition that
> appears in more than one file resolves against it. No consuming file is edited when a figure changes
> — the register is edited, once, and a governance notice appended to every file by ONE command
> declares the register authoritative.**

Properties that make it a mechanism rather than an intention:

| Property | Why it matters |
|---|---|
| **One write per correction, not N** | The propagation failure mode is *"I updated two of the five"*. With one authoritative row there is nothing to miss |
| **Mechanically checkable** | `grep -c` over the package proves every file carries the notice. This round ran that check and published its output |
| **`DR-NC-06`-compatible** | Superseded values stand unedited in their original files; the register names each one and where it stands, so the lineage is *more* visible, not less |
| **Reviewer-legible** | An independent reviewer resolves any figure in one lookup, instead of reconciling five files — and can detect a stale figure that no longer appears in the register |
| **It survives the author** | The next round inherits the register, not a habit |

**Implemented in this package as `MCC_00_CANONICAL_FIGURES_REGISTER.md`**, created in response to the
finding, with the propagation command retained at `LAYER2_MCC_EVIDENCE/mcc_scripts/propagate.sh`.

### Proposed standard delta `D-9`

> Adopt `ER-CORE-3` clause 6. **Reject any deep-research package that publishes a figure in more than
> one file without a governing register**, and require the propagation check output in the evidence
> manifest. Cost: one file and one command per round.

### And one further rule the challenge earned

### `ER-CORE-8` — hash the package before review · `DOMAIN-INDEPENDENT`

The audit panel enumerated **9 files at the start of its review and 20 at the end**; two grew and four
appeared while it worked. **No reviewer verdict on a package can be relied on without a content hash.**

> **Rule: a package handed to independent review is frozen and hashed. The reviewer's report names the
> digest it reviewed. A package that changed under review is re-reviewed or the delta is enumerated.**

This round did not do it, and the panel had to say so. The evidence manifest now carries a per-file
digest and a roll-up digest for exactly this purpose. Proposed as standard delta **`D-10`**.
