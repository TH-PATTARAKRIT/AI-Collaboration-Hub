...
| `CI-12` | deletion path → ~~**installed in all three deployed databases**~~ → **installed in all FOUR frozen RC-05 extracts**, two generations **[`P11-C6-04`, 2026-09-08 — population premise re-pointed to P08 `82df5f3df8faec484a27146eb1f1d8b652424293`.** The deletion module records an **installed** state in **all FOUR frozen RC-05 extracts**, read from each extract's own module registry. **Four frozen extracts are NOT an established deployment census** and P11 does not upgrade them into one. **`exercised` remains NOT ESTABLISHED** — install state is capability, not act.]** |
