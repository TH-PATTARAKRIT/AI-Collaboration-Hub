...
|---|---|---|---|
| `DC-12` | **Orphan financial facts / facts without provenance** | Fact `F7` **provenance is not implemented at all**. A manual journal to a control account creates a financial fact with no source. `AE-14` opening entries are ordinary entries with no migration identity — element 14 fails 10/10 | Design-side, **root-independent** |
