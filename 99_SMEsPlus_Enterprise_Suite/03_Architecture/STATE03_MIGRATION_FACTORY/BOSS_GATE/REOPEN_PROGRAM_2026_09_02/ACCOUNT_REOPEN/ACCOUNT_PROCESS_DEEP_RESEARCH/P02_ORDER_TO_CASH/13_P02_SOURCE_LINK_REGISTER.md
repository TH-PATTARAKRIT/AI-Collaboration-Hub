...
| 112 | `R19/stock_account/models/res_company.py:135-141` | …and its cron domain. **CORRECTED after independent challenge:** the month-end branch combines with **`&`**, not `\|`, so the domain becomes `inventory_period='daily' AND inventory_period='monthly'` — **unsatisfiable**. A `monthly` company never auto-closes, and at month-end even `daily` companies are skipped. The original register entry read this as "widened" and was **wrong**. Filed as reference-ERP defect **D-01** in `25` §3. |
