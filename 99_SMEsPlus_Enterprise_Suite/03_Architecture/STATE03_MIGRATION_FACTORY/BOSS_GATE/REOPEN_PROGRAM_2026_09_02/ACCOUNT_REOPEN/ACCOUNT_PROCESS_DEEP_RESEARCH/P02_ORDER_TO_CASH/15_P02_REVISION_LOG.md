...
| **RE-32** | The zero-COGS invariant holds over 2,553,914 lines across 4 generations. | **80.69% of that denominator is a generation where the marker is not in the field's selection.** v14 `display_type` admits only `line_section` and `line_note`. Re-scoped to **493,277 marker-capable lines, 15 databases, generations 16/18/19** — of which 90.7% is one database. | AAS-03 Expert 2, with a positive control on the same join. | **Maximum. This is the package's headline.** |
| **RE-33** | `database.uuid` is the identity key. | **It is a restore-lineage key.** Seven differently-configured live databases share one uuid; two live 19.0 uuids sit outside the population. **≥19 uuids, ≥26 instances.** | AAS-03 Expert 2. | **Maximum.** The key adopted to defeat name-collapse commits the inverse error inside our own population. |
| **RE-34** | Outcome 3 was *measured, not inferred from configuration*. | **Entailed by the configuration already published.** NULL valuation → `manual_periodic` → no entry **by design**. Corroboration, not independence. Sharper unpublished finding promoted: **15 of 126 categories fully account-configured with valuation never set**. | AAS-03 Expert 2. | **High.** The figure stands; the claim about what it establishes did not. |
| **RE-35** | The artefact population is 39, closed, confirmed by two instruments. | **At least 40.** The sweep is blind to plain-SQL dumps, and `22` had **already added** the missing one by explicit correction. Both confirming instruments shared the blind spot. | AAS-03 Expert 2, with a positive control on a known plain dump. | **Maximum.** A later round silently reversed an earlier round's recorded correction. |
| **RE-36** | `P02-F-28a`: `iEVING` proves uuid-keying beats name-keying. | **Refuted on its own example.** One `ir_config_parameter` row, `create_date` identical to the microsecond, uuid rotated 5 minutes after a backup — **one lineage, not two databases**. The name key would have been right. | AAS-03 Expert 2. | **Maximum.** The package's sole worked argument for its identity key demonstrates the opposite. |
| **RE-37** | A file is a PGDMP archive if it begins `PGDMP`. | **Magic prefix is not validation.** A file beginning `PGDMPX` was counted. Now requires the archive TOC to parse (`pg_restore -l`). | This round's **failure control**. | Medium — it would have inflated the artefact count. |
| **RE-38** | A zip is an Odoo backup if any member ends `/dump.sql`. | **Looser than the rule it replaced.** Matched a Gantt-library sample file and a 57-byte Magento test fixture. Now requires `dump.sql` at the **archive root**, or `<root>/dump.sql` **with** `<root>/manifest.json`. | Inspecting the hits rather than the count. | High — two false artefacts entered the population and were removed. |
| **RE-39** | The declared `$HOME` path set was complete. | It **omitted `~/Library/Group Containers`**, which holds **4 artefacts of the known population**. | **Regression check against the prior population** — comparing the new sweep's output to the old. | High. A format-complete method with an incomplete path set is not complete. |
| **RE-40** | 38 artefacts and 4 others are unreadable (`BLOCKED_IO`). | **`timeout` does not exist on this host**; the wrapper returned command-not-found and empty output, which the code read as blocked. | Testing the wrapper itself after an implausible universal result. | **Maximum.** It would have published a total inversion — *every* artefact unreadable — and an absent command and an unreadable file produce the identical empty string. |
| **RE-41** | `C-43`'s *"15 databases"* and this round's *"15 lineages"* are the same 15. | They are not. 15 uuids vs 15 lineages; **marker-capable lineages = 13**. | Checking the two figures against each other before reusing either. | Medium — a coincidence of numbers that would have propagated as an identity. |
| **RE-42** | The amended lineage key (birth metadata + founding company) identifies a lineage. | **Disproved in both directions.** Clause (b) produced a **false split** on `pfp-main`/`pfp-staging`, which content ancestry, `database.secret` and uuid **version** all show to be one lineage. Clause (b) is a current-state field and collides both ways in-estate. **LINEAGES = 14.** | AAS-03 Experts 2 and 3, by disjoint instruments. | **Maximum.** Two successive amendments each fixed an over-count and introduced the opposite error. |
| **RE-43** | `SNAPSHOTS ≥ 24`; `ARTEFACTS = 40`; *141,235 content-tested*. | All three wrong. **Snapshots underived (max 17)**; **40 is a path count** (28 distinct contents, one file at 9 paths); **content-tested is 116,977**, with **24,258 read failures** mislabelled as 186 and **807 above-floor files unreadable**. | AAS-03 Expert 2. | **Maximum.** The coverage assertion — the control that exists to prevent exactly this — was itself wrong. |
| **RE-44** | The identity model's six levels were sufficient. | **COMPANY was missing**, under a PLATFORM/TENANT/COMPANY constitution. One database holds **4 roots, 40 branches, three unrelated business groups**; a root-delegated field was counted per company; figures cross a 1→2→44 company change untreated. | AAS-03 Expert 3. | High. Level `1.5b` added with continuity and delegation rules. |
| **RE-45** | `RE-30`'s correction was applied to the registers. | **It never reached `13`**, the register every file cites, which still carried three refuted v18-only negatives; and `07`'s body still asserted the 27-account enumeration beneath its own banner. | AAS-03 Expert 3. | High. **Third occurrence of *a revision log is not a correction* in this programme.** |
| **RE-46** | The `C-04` Boss authorisation pack was safe to submit. | **It forbade companies 2–5 and named a script whose line 4 is `COMPANY_ID = 2`.** Approving it would have authorised a committed write to a real-entity company record. Also: the soft-mode script it names does not exist; the named script's instruments are posted-only and blind to the draft state under test; and the restoration proof cannot fail. | AAS-03 Expert 4. | **Maximum. This is a safety defect, not an accuracy defect.** Pack withdrawn. |
| **RE-47** | `C-04a` closes independently of `C-04b`. | `_post` refuses an already-posted move, so the double requires the move to be **still in draft** — which is the `C-04b` precondition. Proved at function level, stated at system level. | AAS-03 Expert 4. | High. The split was presented as cleaner than it is. |
| **RE-48** | Reachability measured by installed module name; six lineages scored "none". | **False for all six.** `account`'s own validate wizard is a UI-bound soft-mode poster with `force_post` defaulting False, and `account` is installed **11 of 11**. | AAS-03 Expert 4. | High — and it **understated** severity. |
| **RE-49** | *(REVERSAL)* No 14.0 or 16.0 reference distribution exists on this host, so the deployment carrying 90.7% of the evidence has unreadable standard source. | **False.** Full 14/15/16/17/18/19 distributions sit under `~/Library/CloudStorage` — **v14 796, v16 950 module dirs** — pruned by every sweep because of a blanket `~/Library` exclusion adopted for TCC reasons and never declared as evidence-affecting. Standard union 1,634 → **1,932**; non-standard rows **620 → 540**; **`iSMEs` 13 → 1**. | P02, while verifying an expert claim that shared the same blind spot. | **Maximum, and POSITIVE.** `C-55` withdrawn; the evidence base is materially stronger than published. |
