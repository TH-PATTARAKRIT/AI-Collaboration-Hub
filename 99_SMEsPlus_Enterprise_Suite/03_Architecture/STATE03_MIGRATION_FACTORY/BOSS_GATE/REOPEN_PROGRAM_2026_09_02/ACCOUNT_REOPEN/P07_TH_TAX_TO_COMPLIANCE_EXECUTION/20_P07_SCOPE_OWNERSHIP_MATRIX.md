...
| 6 | `account.withholding.tax` | **split**: statutory half `PLATFORM`, GL binding `COMPANY` | `COMPANY` | `COMPANY` | `COMPANY` | `COMPANY` | yes (write-off posting) | the company owning the payment | One model holding name / rate / form tags / `type` **and** `account_id` + `company_id` (`l10n_th_withholding_tax/models/account_withholding_tax.py:11-26`). | **SCOPE CONFLATION, and ownership discarded rather than unprovable.** One record answers to two owners. `company_id` is taken from the acting session (`account.py:82-99` builds vals with no `company_id`), **although the source tax carries a required `company_id`** (`account/models/account_tax.py:126`, with `_check_company_auto = True` at `:76`) and `rec.company_id` was in scope on the loop variable. So the correct charge is not that ownership could not be proven — it is that **provable ownership was overwritten with session context**. Three further consequences: `account_id` has no `check_company=True` and the model has no `_check_company_auto`, so a company-A record may hold a company-B general-ledger account while `account_id` is itself sourced from company B's repartition (`account.py:77`); the update branch `wt_tax.write(vals)` (`:95`) mutates a possibly foreign-company record without proving ownership; and the upsert key `_get_wt_by_tax()` (`:41-46`) searches with `active_test=False` and no company predicate, so it changes shape with the acting user's allowed-company set. `P07-F-18` |
...
| 9 | Tax point field (`account.move.tax_period`, `account.move.line.tax_period_date`) | `COMPANY` | `COMPANY` | `COMPANY` | `COMPANY` | `COMPANY` | determines the period of a financial effect | the company of the move | `smesplus_tax_period_date/models/tax_period.py:23`, `:36`. | **CORRECT scope, INERT function.** The line-level field is written only in `create`; no report, compute, domain or SQL reads it, and its sole reader is a hidden list column. The header field is displayed but does not drive period selection. `P07-F-02` `P07-F-03` |
...
| 13 | Withholding tax certificate (s.50 bis) + lines | `COMPANY` | `COMPANY` | `COMPANY` | `COMPANY` until issued | `TENANT` (the payee holds a copy) | evidences a financial effect | the withholding company | `withholding_tax_cert.py:148-155` (header), `:403-405` (line — a **non-stored** related field). Two record rules exist, but the line rule is a **redirect to the parent, not an independent control**: it resolves through `cert_id.company_id`, so it is redundant with the header rule, its `('company_id','=',False)` limb is dead for an orphan line, and it adds a correlated subquery to every access on a model the PND wizards read per row. An earlier draft's "record rules present for both" overstated this. | **OWNERSHIP CORRECT, ACCESS OVER-BROAD.** Unlink on the certificate and its lines is granted to the billing group; the creation wizard and both WHT report models are granted to `base.group_user` with **full create/write/unlink**, not read. `l10n_th_withholding_tax/security/ir.model.access.csv` additionally grants **unlink on `account.withholding.tax` itself** to the billing group — the statutory-half record of row 6. One mitigation, stated so the finding is not overdrawn: the cert module uses no `sudo()`, so the wizard grant does not by itself let a plain user create a certificate. Access scope exceeds object scope. `P07-F-19` |
...
| `P07-U-14` (**CLOSED as a finding, no longer a HOLD**) | The tenant boundary is **specified and not built**. `99_SMEsPlus_Enterprise_Suite/FR_DETAIL_TENANT_MANAGEMENT.md` (status `Draft Baseline`) registers `FR-TEN-001`…`FR-TEN-009` as status `NEW`, including `FR-TEN-008` "System isolates data by tenant scope"; `MODULE_SPEC_TENANT_MANAGEMENT.md` accompanies it. A search of the **entire volume** for a tenant ORM model (`_name = '<...>tenant'`, all Python files) returns **zero** files. So nothing contains the unbounded company search, because the containing construct does not exist yet anywhere. `P07-F-50`, class `A — VERIFIED ABSENCE`, boundary: all `.py` on `/Volumes/iMacSys`. |  `A — VERIFIED ABSENCE` |
