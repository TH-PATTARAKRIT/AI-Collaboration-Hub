...
| `P07-U-32` | The **22 database artefacts** (corrected from 36 at `P07-F-93`) under the `~/Library` subtree the published census pruned (`P07-F-92`). Unkeyed, unread, not counted as identities. Keying them on `database.uuid` is the largest single unexamined evidence set this package has named. | `NOT YET READ` | `22 §35.4` |
