...
| `P07-F-86` | `S1` | **Five of seventeen negatives were bounded by a scope this session later proved incomplete, and none was re-run when the boundary moved.** `P07-N-01`, `-N-02`, `-N-06` are bounded by the declared PATH SET — proven not the deployed set (`P07-F-68`, 85 installed modules outside it). `P07-N-03` and **`P07-N-25`** are bounded by `/Volumes/iMacSys` — proven incomplete by `22 §13`, since `$HOME` holds addon trees and 8 of the 15 database snapshots. **`P07-N-25` underpins `P07-F-50`** (tenant boundary specified-not-built): its boundary was *all `.py` under the volume*, and **`$HOME` was never searched**. Re-run over the corrected root set was launched, then **superseded** by the `ir_model` registry route that settled `P07-N-25` at `22 §30`/`§31`, and the walk itself **terminated unfinished** — `TERMINATED — REDUNDANT CORROBORATIVE ROUTE / BLOCKED_ON_IO`, `22 §48.1`. `P07-F-50` stands **strengthened** on the registry route. **`P07-N-03` had no re-run and closes the round unresolved.** Counter-case worth the record: the four negatives written with a **narrow** declared boundary are the ones that did **not** go stale. | own boundary audit | `22 §28` |
| `P07-F-87` | `S2` | **The residue has two classes, and the second is unreachable by any model-level analysis.** Three exposure routes — models (`P07-F-83`), findings (`P07-F-83`), negatives (`P07-F-86`) — converge for model-scoped claims: `P07-N-01`, `-N-02`, `-N-06` sit on `account.move`/`.line`, the same models the finding route flagged, so the model-scoped residue is **one coherent surface**: 3 modules, 5 findings, 3 negatives. But **`P07-N-03` and `P07-N-25` concern no model at all** — *"no tenant ORM model exists anywhere"* is a claim about the **absence of a class of model**, which no *which-module-declares-on-which-model* analysis can bound. All ten of P04's negatives are module-bounded, so its convergence covered every one; two of this package's seventeen are whole-product, and they are exactly the two left over. Each needs its own re-run. | own convergence test | `22 §29` |
| `P07-F-88` | `S1` | **`P07-N-25` SURVIVES on a stronger basis, and `P07-F-50` stands strengthened.** Tested against each deployment's own model registry — a route needing **neither module map nor path set**, which is what `P07-F-87` said made the claim unreachable: **5,133 model rows across 7 of 7 identities and 3 generations, zero tenant-shaped models** — corrected at `22 §31` from a first run that covered **5 of 7** and used only positive controls. Positive control `res.company` fires in all 7; **negative controls `account.tax.unit` and `withholding.tax.cert` correctly report ABSENT in the never-transacted lab**, so the instrument distinguishes a real absence from a failed read. Re-scoped per `REV-M-56` and **not re-published in whole-product form**: bounded to the **seven** registries, with both limits carried — it bounds the **estate on this host, not the product**, and **absence of a model is not absence of a behaviour**, so it is decisive only for a missing **record type**, which is how the claim is phrased. Near-miss check: **`scgl.multi.approve.organization`** would have falsified a looser *no organisational-boundary construct* phrasing — the inversion demonstrated on this package's own material. | runtime, 5 identities | `22 §30` |
| `P07-F-89` | `S2` | **The absences that license `§31`'s instrument were themselves unverified, and are now confirmed by three independent single runs.** `a6664233`'s negative controls are absences *inside an otherwise-successful table* — P04's newly reported failure mode, where a batch harness returned *no block* for three of eight identities that single runs disprove at 1,080 / 510 / 633 models. Re-tested here: `ir_model` (control `res.company` present), `ir_module_module` (**no row at all** for the cert module; controls `base` installed `18.0.1.3`, 41 modules, `account` installed) and the archive TOC (control `account_move_line` present) **all agree**. A `§25` absence spot-checked the same way: `account.tax.group` carries only `account` and `point_of_sale`, harness read counts 62,445 / 16,009 / 3,516 printed, positive control 75 modules on `account.move`. **Caveat stated, not smoothed:** `account.tax.unit` is absent although `account` is installed there; why is **not established** and not needed for the control's purpose. | 3 instruments, single runs | `22 §32` |
| `P07-F-90` | `S2` | **`22 §13`'s census declared its roots and not its exclusions — the published "15 snapshots · 7 identities" is a floor.** Two undeclared narrowings: the walk pruned **eight directories** including **`Library`**, where `~/Library/CloudStorage/` holds the Google Drive trees this package's own `§19` enumeration drew dozens of module copies from; and the ZIP signature required a member literally named `dump.sql`, narrower than declared. P04's census, run with wider criteria and no such pruning, reports **39 artefacts** against this package's 15. Re-run over the excluded subtree with the widened signature **returned** — it is the run reported at `22 §35` (36 artefacts), reconciled to 22 at `§36` and 20 at `§37`; **no `§13` number was amended and `§13` closes the round as a floor** (`22 §48.2`). **No finding moves**: every database-derived finding is bounded to a named identity and all seven were keyed on `database.uuid` and read. Third undeclared narrowing this package has recorded **after** publishing the thing it narrowed (`P07-F-79`, `REV-E-74`, this). | own script audit, prompted by P04's census | `22 §33` |
| `P07-F-91` | `S2` | **The census signature set was declared at three and executed at two.** `22 §10.7` published it as `PGDMP`, ZIP-containing-`dump.sql`, and *"gzip-inspected-and-excluded"*; `§13.1` published two; the census script has **no gzip branch at all**, and the inspection `§10.7` refers to covered **2 files** in an earlier narrower sweep, not the declared roots. Worse than the peer's equivalent: theirs ran and was too tight, **this one was described and never ran over the census population** — the package's own *declared-pattern-not-run* defect recurring in the section that declares the method. Also confirmed: `os.walk(onerror=lambda e: None)` suppressed every unreadable directory with **no count ever taken**. Control built before the result: a known `PGDMP` dump gzipped decompresses to `PGDMP` with **no** identifying text, so the branch fires and a text-only test would miss it; a genuine source tarball matches neither. Both re-runs executing; **no `§13` number amended until they return**. | own script audit | `22 §34` |
| `P07-F-92` | `S1` | **The undeclared `Library` exclusion dropped more artefacts than the declared census found: 36 against 15.** Measured over the pruned subtree; disjoint from the census by construction; floor by file now **≥ 51**. Almost all sit under `~/Library/CloudStorage/GoogleDrive-*/`, the trees this package's own `§19` enumeration had already read dozens of module copies from, and one matches an artefact P04 named as newly visible — **the two sweeps agree on a file this one could not see.** Under the `REV-M-66` constraint, committed before the result: **the file count is superseded, the identity count is NOT** — the 36 are unkeyed, so `7 identities keyed and read` stands and **population state is OPEN**. No finding moves: every database-derived finding is bounded to a named identity and all seven were keyed on `database.uuid` and read. `P07-F-01`'s prevalence is now explicitly *1 of 4 **examined***. `P07-U-32` opened — the largest unexamined evidence set this package has named. | own re-run of the excluded subtree | `22 §35` |
| `P07-F-93` | `S2` | **`P07-F-92`'s 36 is corrected to 22, and the two sweeps then reconcile exactly.** `§35`'s scan accepted a zip containing **any** `.sql` member; re-classified, **10 `PGDMP` + 12 zip-with-`dump.sql` = 22 real, plus 14 source-code archives** (`odoo-16.0.zip`, `01 ACCOUNT.zip`, …) matched on a **0 KB `neutralize.sql`** shipped inside a module. P04, independently bounded, reports **22**. Floor by file **37**, not 51. The `manifest.json` clause this package first suspected contributed **0** — the wrong guess made in the same breath as the right method. **Severity ranking unaffected (22 against 0); identity position unchanged (7 keyed, population OPEN); `P07-U-32` renumbered to 22 unkeyed artefacts.** First correction in this exchange to **reduce** one of this package's numbers. | own re-classification, after P04 declined to adopt the count and asked for paths | `22 §36` |
| `P07-F-94` | `S2` | **22 → 20: the name test cannot express the distinction; the shape test can.** P04's discriminator — **`dump.sql` at the archive ROOT with `manifest.json` beside it**, the shape the product writes — applied here finds the **same two source archives** it withdrew: `CFF.zip` (`dump.sql` is Gantt-library sample data) and `docker-compose-magento.zip` (a Magento acceptance-test fixture), both **nested-only**. Corrected composition: **10 `PGDMP` + 8 backup-shape + 2 root-dump-no-manifest = 20**, the last two kept and **declared as a distinct export shape** per P04's own narrow-pattern reasoning. Figure has moved **36 → 22 → 20**, downward each time, each time on a sharper peer discriminator. **Nothing downstream changes**: `P07-U-32` renumbered to 20, identity position untouched (7 keyed, OPEN), severity ranking **20 against 0**. Backup-shape names cross-validate P04's newly-visible list — `iSMEs182`, `iSMeO2C`, `iMSCG` ×2, `pankhamhom` — **named, not counted as identities.** | own structural re-test | `22 §37` |
| `P07-F-95` | `S3` | **The shape test applied to `§13`'s own zip candidates — 15 stands, basis replaced.** `§37` ran the test on the `~/Library` subtree and not on the census that had admitted its two zip members by the same **name** clause `§37` rejected. Run: both are **root `dump.sql` + root `manifest.json`**, control (`odoo-19.0.zip`) correctly rejected; the other 13 are `PGDMP`, where the magic **is** the shape. **Fourth instance in this thread of a count surviving while its basis was replaced.** Census finally composed: **23 `PGDMP` + 10 backup-shape + 2 declared-shape = 35 by file**, 4 rejected — **35, not 37**, `§36` having carried its arithmetic one section past `§37`'s correction. Identity position unchanged: **7 keyed, OPEN, 20 unkeyed at `P07-U-32`.** | own systematic re-run | `22 §38` |
| `P07-F-96` | `S2` | **Every re-audit in this thread tested what the census ADMITTED; none tested what it could have MISSED** (P04). A backup whose payload is named other than `dump.sql`, with a root `manifest.json` beside it, **was never a candidate** — and **tightening a predicate can only shrink what it admits**, so `§36`, `§37` and `§38` could each confirm the census was *not too loose* and **none of them could show it was not too narrow**. Controls built first: a known backup is **correctly excluded** by the inverse predicate (it carries both), and a **synthesised** archive with root `manifest.json` + payload `backup.sql` **fires** — so the class is real, constructible, and was unreachable by every predicate this package published. Inverse sweep **complete: 3,133 archives examined, 1 unreadable, ZERO of the inverse shape** — the census is **not too narrow** in this direction either, and with `§37`'s rejections the artefact set is bounded **in both directions**. Residue declared, not suppressed: **1 unreadable of 3,133**, a bound of one rather than a zero. `§38`'s numbers stand; identity position untouched (**7 keyed, OPEN**). | own run of P04's complement test | `22 §39` |
| `P07-F-97` | `S1` | **`§39`'s declared residue of one has a name, and it was missed twice by two different defects.** `BK12MAY26_2026-06-23_13-15-32.zip`, 1.08 GB under `~/Library/CloudStorage/…`: **materialised** (allocated ≥ logical, not a placeholder), leading bytes `50 4b 03 04`, **end-of-central-directory absent**, `BadZipFile`. **Load-bearing**: `BK12MAY26` is keyed identity `66d1b52a`, held only at **2026-08-03 ×2**; this is **2026-06-23**, a date held for **no** keyed identity — so it is **either a third snapshot of `66d1b52a` or an unkeyed identity**, undeterminable without reading it. Missed (1) by the undeclared `Library` prune and (2) by `§35`'s `except Exception: pass`, which **silently dropped** it rather than counting it unreadable — *the first defect removed it from the population, the second removed the evidence that anything had been removed.* Only `§39`'s complement sweep counted it, because that run counted its own failures. **New disposition class `PRESENT AND UNREADABLE`; `P07-U-33` opened, sole member.** No number amended. | own diagnosis, prompted by P04 naming the same file | `22 §40` |
...
| `P07-F-103` | `S3` | **Two of seven sweep units carried a declared denominator where a derived one belongs — including the unit beside the one that derives.** P04's rule: *fixing a rule in one unit does not propagate it to the next unit written; each unit carries its own denominator.* Audited: `[0]`, `[1a]`, `[2]`, `[3]`, `[4]` derive; **`[1]` read an `OWNED` literal of 33** and **`[5]` a scrub-token literal of 18**. Results differ in kind: `[1]` derives **33 = 33**, the literal complete **today** — number unmoved, basis replaced; **`[5]` derives 78 against 18**, so the Layer-1 scrub was checking **less than a quarter** of the vendor-, module- and identity-shaped tokens present in Layer 2. **The pack is still clean against all 78** — nothing published was wrong, but the guarantee covered far less than it claimed. Third instance of *a narrow check whose result survives widening*, after this package's `Library` exclusion and P04's gzip branch — **and none of the three could be known to cost nothing until run.** | P04's derived-denominator rule, audited across all seven units | `22 §46` |
| `P07-F-104` | `S2` | **`P07-F-103`'s fix was wrong: replacing the scrub literal with a derivation dropped 5 of 18 tokens, on the half that matters most.** P04: **a vendor name has no shape** — an ordinary lowercase word, derivable only from knowing what the vendor is called — so a derivation **silently drops exactly the class clean-room exists to catch**. Measured here: declared 18, derived 78, **shared only 13**; the five lost are the prefixes `l10n_`, `scgl_`, `smesplus_` (which match **any** module carrying them, including modules Layer 2 never mentions) and the bare identity names `idemo18`, `occ_sim`. **For one commit the guarantee was weaker than the one it replaced while the reported figure grew fourfold — coverage moved opposite to the number.** Corrected to `declared ∪ derived` = **83**, with both differences reported; Layer-1 pack **clean against all 83**, so nothing published was wrong. **A derivation and a declaration are not two attempts at the same set** — the literal is the only record of what cannot be derived. | P04, correcting this package's fix and asking for the intersection | `22 §47` |
...
- **`PRESENT AND UNREADABLE`** — the evidence exists, is materialised on this host, and is not
  recoverable without **changing the estate**. Added at `22 §40.3`; one member.
...
| `P07-U-33` | **`BK12MAY26_2026-06-23_13-15-32.zip`** — 1.08 GB, materialised, structurally damaged (no end-of-central-directory), name collides with keyed identity `66d1b52a` at a date held for no keyed identity. Either a third snapshot of it or an unkeyed identity. Reading it requires repairing the archive — **a change to the estate, outside this brief.** | **`PRESENT AND UNREADABLE`** | `22 §40` |
| `P07-U-32` | The **20 database artefacts** (36 → 22 → 20, `P07-F-93`, `P07-F-94`) under the `~/Library` subtree the published census pruned (`P07-F-92`). Unkeyed, unread, not counted as identities. Keying them on `database.uuid` is the largest single unexamined evidence set this package has named. | `NOT YET READ` | `22 §35.4` |
| `P07-U-31` | `P07-N-03` — *four Thai tax-document modules absent from the declared set* — is a claim about **module presence in a tree**, not a record type, so the registry route does not settle it. Re-run over the corrected root set. **Now the only member of the whole-product residue.** | `NOT YET READ` | `22 §30.4` |
...
**31 items: 4 closed, 23 `NOT YET READ`, 1 `NOT ON THIS HOST`, 2 `EVIDENCE NEVER RECORDED`, 1 `PRESENT AND UNREADABLE`.**
