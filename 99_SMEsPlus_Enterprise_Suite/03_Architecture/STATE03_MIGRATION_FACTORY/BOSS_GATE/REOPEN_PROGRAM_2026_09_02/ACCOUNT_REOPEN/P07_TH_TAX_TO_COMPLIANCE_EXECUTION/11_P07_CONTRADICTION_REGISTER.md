...
| `P07-C-26` | A peer's `FACT VERIFIED` against this package's measurement. | **P10** (`1fea562`): a tax-period carrier exists on the entry and is **absent from the item**, deployed 19.0+e | **P07** (`EP-3`, `EP-4a`): both carriers are **co-present in 3 of 7 identities and co-absent in 4**, one module defines both, and one of the three is **v16** | **RESOLVED in P07's favour on evidence**, by two independent routes. Handoff published at `S02_DOMAIN_PURE_CLOSURE_2026_09_06/P07_TO_P10_HANDOFF.md §5`. **P07 does not edit a peer's package**; the correction is P10's to make. |
| `P07-C-27` | A discharge read wider than its scope. | `P07-F-78`: *"which copy is deployed cannot change any published finding"* | `P07-F-108`: the two copies **disagree** on `_compute_wht_amount`, and a new claim turns on it | **NOT A CONTRADICTION — a scope boundary made explicit.** `P07-F-78` stands as written: it tested four cited claims and both bodies agreed on all four. **A discharge is scoped to the claims it tested.** `REV-M-96`. |
...
| `P07-C-29` | A disproof attempt against this round's most consequential finding. | `CH-B`: one withholding-rate edit after posting would produce all 199 mismatches, making `P07-F-109` an instrument artefact | `EP-6b`: **151 of 199 are moves whose own lines carry no withholding tax at all**, and the other 48 show no dominant ratio where one rate edit predicts one | **CHALLENGE FAILED ON MEASUREMENT.** `P07-F-109` stands and is stronger. `CH-B`'s residual disagreement — that the *cause* is still unproven — is upheld and is what `P07-U-35` records. |
