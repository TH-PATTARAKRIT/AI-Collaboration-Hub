...
| `A-06` | Excess input tax | Carry forward or claim refund; refund within 3 years of the filing date | `S-18` `S-34` | **Implemented**: line `10_EXCESS_CARRIED_FORWARD` uses an `external` carryover expression scoped to `previous_return_period`, plus a tax-tag expression; lines 11 and 12 consume it. `U-12` **CLOSED**. | The carryover is only as sound as the period it looks back to, and that period comes from an editable company setting rather than from the statute (`P07-F-40`). No refund-claim representation was found — class `B`, boundary as `A-13`. |
