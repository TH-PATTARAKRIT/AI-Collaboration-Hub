...
| ID | Delta | Effect |
|---|---|---|
| `D-A` | **Six real `ir_module_module` registries were located and read offline.** `U-01` moves from *no evidence* to *resolved **for the six registries named** (class A within them), HOLD for the v18 target and class **B** for any wider population*. | `24` |
...
| `D-D` | **`scgl_purchase_advance_payment` and the whole Thai WHT stack ARE installed in all four distinct databases evidenced.** The findings in those modules move from conditional to **live**. | `26`, `28` |
| `D-E` | **`TX-13` and `TX-20` were empirically tested at production scale — and BOTH were published wrong, then contradicted and corrected by independent review.** Corrected: **one** exact duplicate certificate exists in 5,201 (not 32 — the rest are one-per-payee on bulk runs, rate splits, or drafts); and the certificate's printed `date` is **correct in 97.79%** — the create-time artefact is on the column named `payment_date`, which carries no payment information in **100%** of rows. Both corrections author-verified against the data before adoption. | `25 §3`, `39 RE-10`, `RE-11` |
