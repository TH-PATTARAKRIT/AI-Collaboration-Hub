...
`scgl_purchase_advance_payment` is **installed in all five real business databases** evidenced
...
|---|---|---|---|
| `H-P07-1` | **Two independent WHT subsystems are installed together in 4 of 5 real databases.** The enterprise PND CSV export derives its amount by joining `account_move_line.tax_line_id` to `account_tax`; the custom stack produces WHT as a payment **write-off** line. The custom line carries the matching `tax_tag_ids` but no `tax_line_id`. | Does that divergence affect a filed return? | `l10n_th_reports/models/tax_report_pnd.py:29-64`; `l10n_th/data/account_tax_report_data.xml:192,203,...`; `l10n_th_withholding_tax/wizard/account_payment_register.py:21-25` |
| `H-P07-2` | **32 payments carry more than one non-cancelled certificate; one carries nine.** Only 5 of 5,201 are cancelled and only 6 carry a substitution reference. | Is a payment permitted multiple live certificates? | `25 §3` |
| `H-P07-3` | **4,081 of 5,201 certificates (78.5%) are dated other than their payment date**, in a +1/+2/+3-day distribution consistent with a create-time default. | Which date governs the filing period? | `25 §3` |
