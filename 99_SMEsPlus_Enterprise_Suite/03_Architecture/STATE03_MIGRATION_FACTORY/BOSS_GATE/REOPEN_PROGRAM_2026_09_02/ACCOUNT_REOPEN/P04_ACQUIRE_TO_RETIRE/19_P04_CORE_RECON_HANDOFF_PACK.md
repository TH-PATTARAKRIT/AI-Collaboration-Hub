...
| The statutory safe harbour for applying company property to internal use covers **use, not transfer**, and only within a business that is itself liable to the tax | So the retirement path that records a donation, a scrapping or a write-off is **outside the shelter**, and the exposure below is not relieved by it |
