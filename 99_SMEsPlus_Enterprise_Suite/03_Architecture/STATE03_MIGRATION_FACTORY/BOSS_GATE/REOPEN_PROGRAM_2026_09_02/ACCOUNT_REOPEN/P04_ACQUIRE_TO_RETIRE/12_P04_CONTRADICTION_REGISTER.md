...
| **P04-CTR-08** | Model policy is applied by **three call sites** and by **no** property of the record; `create()` applies nothing | Med | **OPEN — design.** `P04-F-145`. Latent vs live is `P04-B-55` |
| **P04-CTR-09** | **Ten snapshot fields and one live field** on the same object: editing a model's not-depreciable percentage restates every linked asset, editing its method restates nothing | Med | **OPEN — Boss design choice.** `P04-F-146`; three options stated at Design Pack `DF-02` |
| **P04-CTR-10** | **Two fiscal-lock behaviours in one module**: disposal **raises**, account re-assignment **silently skips** | High | **OPEN.** `P04-F-155`. Routed to **P08** |
| **P04-CTR-11** | A Boss-approved policy input (off-balance pairing) is **unimplementable on `account.asset`** — all three account fields exclude that type by domain | High | **OPEN.** `P04-F-154`. Policy stands; the route does not. Routed to **P08** |
| **P04-CTR-12** | Company scope is **asymmetric between two Many2ones in one custom module** | Med | **QUESTION, not a claim.** `P04-F-156` → `P04-B-54`; record rules not examined |
