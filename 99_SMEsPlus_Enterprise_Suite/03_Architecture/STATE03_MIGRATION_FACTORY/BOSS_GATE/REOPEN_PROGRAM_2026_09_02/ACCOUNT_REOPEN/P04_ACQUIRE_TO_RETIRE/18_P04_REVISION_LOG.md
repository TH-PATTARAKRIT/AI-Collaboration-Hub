...
| **P04-REV-68** | **P07's complement sweep returned zero across 3,133 archives with 1 unreadable, and it declared that one as a bound of one rather than a zero.** Asked of my own sweep, which had reported a bare zero: **I had not counted unreadable archives either.** Counted: **1 of 1,002 zips ≥ 1 MB** — and unlike P07's, **mine is named for a database**. `BK12MAY26_2026-06-23_13-15-32.zip`, 1.08 GB. `P04-F-132`. **Diagnosed rather than assumed, which changes what it is.** Materialised — 1,081,946,112 bytes allocated against 1,081,942,526 logical, so **not a cloud placeholder**; leading bytes `50 4b 03 04`, so **a zip**; first 1 MB reads; **end-of-central-directory signature absent**. It is **structurally damaged, entirely on disk, and missing its index**. And the name is load-bearing: `BK12MAY26` is a **keyed identity** (`66d1b52a`) held only at 2026-08-03 ×2, while this is dated **2026-06-23** — so by this package's own rule that a name is not an identity in either direction, it is **either a third snapshot of a keyed identity or an unkeyed one, and which cannot be determined without reading it.**|**It joins the class that research cannot close** — with `P04-B-46`'s two source-less modules: **evidence that exists on this host and cannot be read from it.** Repair of a 1 GB archive is a change to the estate and outside this brief. Registered `P04-B-48`. **`P04-F-126`'s 39 and `P04-F-131`'s zero both now carry a declared bound of one, and neither is amended** — *a zero with a named exception is a different statement from a zero, and the difference is the whole reason for saying it.* This is the only **named** residue either package produced, and a named one is worse than a nameless one because it can be checked against what is already keyed|
| **P04-REV-67** | **`P04-F-131` published a zero with half a control, one commit after this package recorded that exact defect.** What went out was an **exclusion** control — a known backup shows both members and is correctly *not* flagged — which proves the predicate does not misfire and **says nothing about whether it fires**. A zero from a test never shown to detect anything is the **control-that-cannot-fail** pattern recorded at `P04-F-109`, committed in a finding whose entire content is a zero. **P07 supplied the missing half by synthesising the artefact** — a zip with root `manifest.json` and a payload named **`backup.sql`** — and built both controls **before** running its sweep, per its own commitment rule. Run here: the inverse predicate **fires** on it (root manifest 1, root `dump.sql` 0); **the original census clause does not match it**, so the class is real, constructible, and was unreachable by every predicate this package published; and the predicate correctly does **not** fire on a genuine backup. `P04-F-131`'s conclusion is unchanged and **its evidentiary standing is not** — the zero is now controlled in both directions.|**The generalisation, which is the last thing this thread produced:** an **exclusion** control and a **firing** control are different instruments and a negative needs both. Proving a predicate does not misfire is worthless without proving it can fire, and the second is usually only obtainable by **constructing an instance that does not exist in the population** — which is why it gets skipped. P07's rule of building both *before* the sweep is what makes the result reportable whichever way it lands; mine were built after, and only because a peer named the gap|
| **P04-REV-66** | **The census closed in the other direction, which none of the three preceding re-audits could do.** `P04-F-130` tested what the census **admitted**; it never tested what the census could have **missed**. The census keyed on a member named `dump.sql`, so a backup whose payload carried another name — with a root `manifest.json` beside it — would never have been a candidate, and **no shape test run afterwards could find it.** Run: every `.zip` ≥ 1 MB under the declared roots listed and tested for the inverse shape. **Zero.** Control: a known backup shows root `manifest.json` **and** root `dump.sql`, so it is correctly *excluded* from the candidate set — the test can return what it looks for and returns none. `P04-F-131`. **So `P04-F-126`'s 39 is bounded in both directions** under its declared path set, size floor and signatures: nothing admitted that should not have been (`P04-F-130`, two rejected) and **nothing missed that should have been** (zero candidates). Fifth basis replacement in this thread and the first that addresses **omission** rather than **admission**.|**The asymmetry every tightening in this thread shared, now named:** tightening a predicate can only ever **shrink** what it admits, so `P04-F-127`, `P04-F-129` and `P04-F-130` could each confirm the census was **not too loose** and **not one of them could show it was not too narrow**. **A discriminator validates the set it accepted; only its complement validates the set it never saw.** P07's closing round had the same gap in mirror image — it had run the shape test on one subtree and not on its own earlier candidates — and both gaps are the same shape: *the test was applied where the author was already looking*|
| **P04-REV-65** | **The shape test applied systematically to this package's own census — the number holds, the basis changes for the third time.** P07 applied the discriminator to its 22 and reached **20**, **independently finding the same two archives** I had withdrawn by hand: `CFF.zip` (a Gantt library's sample data) and `docker-compose-magento.zip` (a Magento test fixture), both **nested-only**. I had never applied the test systematically to the other ten. Run over all 12: **27 `PGDMP` + 10 root-`dump.sql`-and-`manifest.json` + 2 root-dump-no-manifest = 39**, with the two nested-only rejected and a control confirming a known backup reads the full shape. **`P04-F-126` withdrew two archives on a hand inspection described in prose; they are now rejected by a stated, executable predicate** that also licenses the other ten, and the two kept-but-different exports are **declared as a distinct shape rather than counted silently**. `P04-F-130`. Third time in this thread the count has survived while its basis was replaced — after the 141 exclusions enumerated and the 72 gzip candidates tested at `P04-F-127`.|**P07 reaching the same two archives from the other side is the only instance in this exchange of a discriminator validated by independent application.** Every other agreement between us has been about a **fact**; this is agreement about a **test** — proposed here, applied there, returning the same two rejections out of two different candidate sets. And the distinction that made it work is the whole of what three rounds of tightening produced: ***a name test asks what a file is called; a shape test asks what wrote it.*** Both of us tightened the name twice and were still admitting source archives, because the difference is not in the name|
| **P04-REV-64** | **The disagreement resolved in this package's favour by P07's own re-run — and checking its cause found that I had misdescribed my own predicate.** P07 re-classified its 36 **by why each artefact matched**: **14 were source-code archives**, each caught by a **0 KB `neutralize.sql` shipped inside a module**, one of them a zip of its own declared source set. **36 → 22; under the strict criterion the sweeps reconcile exactly, 22 = 22.** My three candidate tests found nothing because **none was the cause** — the cause was a clause in its scan, not a gap in mine. **`P04-F-126` and `P04-F-127` misstate this package's census predicate**, twice. The zip test did **not** accept *"any `.sql` or a `manifest.json`"*; it required a member **named `dump.sql`**. I had attributed the earlier **reconnaissance** scan's clause to the **census**. Determined behaviourally, not from memory: `odoo-19.0.zip` (73 `neutralize.sql`) and `02_OTHER.zip` (260) **match the loose clause and are absent from the census**, while `CFF.zip` and `docker-compose-magento.zip` **are present** and each contains a member named `dump.sql`. `P04-F-129`.|**The two withdrawals were right for a better reason than I gave, and the transferable half is that name-anywhere is insufficient.** `CFF.zip`'s `dump.sql` is sample data inside a **Gantt charting library**; `docker-compose-magento.zip`'s is a **Magento acceptance-test fixture** — P07's failure mode reached by a different filename. P07 tightened from *any `.sql`* to *a member named `dump.sql`*; **this package was already there and was still admitting two source archives.** The discriminator that separates a backup from source is **`dump.sql` at the archive root with `manifest.json` beside it**, and neither published clause said so. **39 unaffected; the method statement was wrong and is now correct.** Found by executing the clause and comparing to the census output — **a method statement is a claim, and the only way to check it is to run it**|
| **P04-REV-63** | **P07's `~/Library` re-run returned 36 artefacts in the subtree its census had pruned — more than its whole published census — and measured the severity ranking I had argued: its suppressed subtree held 36, mine held 0.** That part is accepted. **What is not accepted is the number as a correction to mine.** My census found **22** in the same subtree. Three candidate causes tested, each with a firing control: a **missing plain-SQL signature** (my census had only `PGDMP` magic and zip-with-`dump.sql`) — **0**; my **≥1 MB floor** — **0**; the **141 suppressed directories** — all OS privacy containers. `P04-F-128`. **The missing plain-SQL signature is a real third undeclared bound in the same script** — after the two at `P04-F-127` — and it **cost nothing in this subtree**. So the disagreement stands: **22 against 36, unexplained from my side.** It could be a broader signature set, a lower floor, a different notion of artefact, or a gap none of my three tests reaches. **What settles it is P07's path list, not its count** — the rule this package applies to every cross-party number: *a joint tally is executable by neither party; two declared halves, each executed by its owner, then compared as paths.* Requested; **39 is not amended in the meantime.**|**One point of agreement, and it is the only cross-validation either census has:** P07 names `premiumflexiblepackaging-pfp-odoo-staging-…` among its 36 and it is in my 39 — **two independently-bounded sweeps agreeing on one artefact**, a stronger basis for that file than either sweep alone. And P07's inversion of `REV-M-67` is the right correction to make explicit: mine was *the number did not move and the basis did*; **its number moved.** *Saying which of the two happened is the whole content of an honest re-audit* — a package that only ever reports the first is not re-auditing, and one that reports the second without a constraint on identity counts is publishing a bigger number. It held that constraint before its result was known, which is why the 36 is reportable as artefacts and its **7 keyed identities still stand**|
| **P04-REV-62** | **Two undeclared narrowings inside this package's own census, found by asking P07's question of it.** P07 found eight directory exclusions in its census script — including **`Library`**, the subtree its own module enumeration had drawn from — and a narrower zip signature, in a section whose whole purpose was declaring the root set. Asked here: **`2>/dev/null` silently dropped every unreadable directory and no number was ever taken** — measured, **141 of 1,070,459 traversed**, all macOS privacy containers (`~/.Trash`, `Library/Accounts`, `com.apple.TCC`); and **the gzip branch decompressed 200 bytes and matched one string**, so a gzipped **custom-format** archive beginning `PGDMP` would have been missed entirely. `P04-F-127`. **Both measured, neither changes the count.** All **72** gzip archives ≥1 MB under the declared roots were re-tested reading 4,096 bytes and matching **either** `PGDMP` magic **or** the SQL header: **none is a database**, with a positive control published — the same test **fires on a known dump after gzipping it**, so the zero is absence and not a dead predicate. **`P04-F-126`'s 39 stands, and stands for a different reason**: it was published on a scan with an unmeasured exclusion and an untested branch, and now rests on 141 exclusions enumerated and 72 candidates individually tested. *The number did not move and the basis did.*|**Third undeclared bound found inside a correction of an undeclared bound**, after `P04-F-113` (the family list, filtered at a frequency floor never declared) and `P04-F-124` (six identities against a census of eight). Stable enough to state as a rule: **a script's declared parameters are the ones its author was thinking about; the narrowings live in the error handling, the defaults, and the branch nobody exercised.** P07's formulation of the same thing is the sharper one for a register: **a census that declares its roots and silently prunes one of them has declared a boundary it did not use**|
| **P04-REV-61** | **The host census completed, and it is published as a count of FILES with the identity count explicitly not stated.** Declared bounds: path set `/Volumes` + `$HOME`, size ≥ 1 MB (justified — smallest Odoo dump on this host is 3.37 MB), two signatures (`PGDMP` magic, zip containing `dump.sql`). Result: **39 database artefacts — 27 `PGDMP`, 12 zip — and 0 reached via the `/Volumes/iMac` symlink.** That last figure closes a claim made on reasoning rather than evidence: I told P07 that `find` without `-L` cannot traverse a symlink to `/` and that my sweep was therefore not inflated. Correct, and **an argument until this run measured it.** `P04-F-126`. **The census pattern was over-inclusive and two artefacts are withdrawn.** The zip test accepted **any** `.sql` or a `manifest.json`; re-tested for the Odoo backup shape, `CFF.zip` and `docker-compose-magento.zip` contain neither and are not database artefacts. Two `premiumflexiblepackaging-*-exact_fs.zip` carry `dump.sql` **without** a manifest — a different export shape, **counted**, because excluding them would be the narrow-pattern error this package has already recorded twice.|**And the unit is the entire point: 39 is a count of FILES.** Only **8** identities are keyed on `database.uuid`. The precedent forbids inference in either direction — `iTEST02` is **four copies of one snapshot**, the two `iEVING` files are **two different databases**. So the census settles the artefact question and leaves the identity question exactly where it was: **8 keyed and read, population OPEN**, with `iErpOCC`, `iSCErP`, `iSMeO2C`, `iSMEs182`, `iMSCG` ×2, `odoo_cff_golive`, two `premiumflexiblepackaging` exports and two archives inside a messaging app's media store newly visible and never keyed by any session. **No finding moves** — every database-derived finding is bounded to a named identity and all eight were read. What changes is the **floor**: this package has read 8 identities out of an unknown total|
| **P04-REV-60** | **The absence that licenses the instrument had itself never been disproved by a second route.** P07 observed that its negative-control rows are **absences inside an otherwise-successful table** — the shape `P04-F-124` names as the most dangerous — and that they are the highest-stakes absences it holds, because they permit the claim that the instrument can tell a real absence from a failed read. It re-tested by three instruments; **the same was true here and had not been done.** `96548e18`'s missing `account.asset` re-tested: **`ir_model`** 510 models, absent, controls `res.company` and `account.move` present; **`ir_module_module`** 1,334 rows, `account_asset` **`uninstalled`**, controls `account` and `base` installed `18.0.1.3`; **archive listing** 0 `account_asset` blocks, controls `account_move_line` 1 and `res_company` 1. Three instruments, three positive controls, three agreeing absences. `P04-F-125`. **This package's version establishes the mechanism, where P07's could not.** Its cert module had **no row at all**, so *why* was undetermined and correctly recorded as not established and not needed. Here the module row **exists with state `uninstalled`** — the asset module is **present in the addons path and deliberately not installed**. Same distinction as `P04-F-97`: **not deployed versus not available**. So `96548e18` is not an incomplete capture but a **complete capture of an install that chose not to run the asset module**, which is a stronger licence for the control than mere absence.|**`REV-M-62` — the three null failure modes now have a set, and naming it is what makes it usable:** the **self-consistent null** (42 empty hashes reading as unanimous agreement), the **control that cannot fail** (all-positive, proving the read worked and nothing else), and the **selective null** (a batch failing on some rows, therefore reading as a finding *about those rows*). P07's addition is the one to keep: **a partial failure will preferentially look like a finding about whichever rows are unusual** — which in both packages means the never-transacted installs, **so the row most likely to be dropped is also the row a broken harness is most likely to indict.** The two biases compound|
| **P04-REV-59** | **Six identities was itself a chosen population, and the two omitted were the informative ones.** P07 found its registry run covered **5** identities against a census of **7**, both omissions being **never-transacted installs**; checked here at once — `P04-F-123` ran **six**, the census holds **eight**, and the two omitted were `a1430edc` and **`f4a44cce`, the never-transacted v19 install**. Same defect, same class of victim, same session. Complete run: **8 identities · 3 generations · 6,308 model rows · zero models of any of the four asserted-absent classes.** `P04-F-124`. **P07's control refinement adopted and it is the sharper half:** an **all-positive** control set proves the read worked and **cannot distinguish an instrument that reads correctly from one that answers *present* to anything**. The table now carries three — a positive (`res.company`, fires in all eight), a discriminating one (`account.asset`, correctly absent in the one identity where the module is not installed), and an explicit **negative** (`account.asset.impairment`, a model that should exist nowhere, absent in all eight).|**Two false absences were produced and neither was published.** A **wrong path** read as an empty result — `unzip` on a non-existent directory, parser reports *no block*, message indistinguishable from a genuine absence; the file was elsewhere and holds **633 models**. Then a **batch harness reported three identities absent that individual runs disprove** — in a loop `4b766580`, `96548e18`, `f4a44cce` all returned *no block*; run singly the same command on the same files returns **1,080**, **510**, **633**. **The second is the more dangerous: a uniform failure announces itself, a failure on three of eight rows inside an otherwise-successful table reads as a finding about those three.** Rule: **always disprove a batch absence with a single run before it becomes a row**|
| **P04-REV-58** | **The registry route extended from one identity to six, after P07 ran it across five of its own and settled `N-25` with it.** `P04-F-122` was bounded to `551ab874` and published with that as a stated limit. Extended: **6 identities, 3 generations, 4,640 model rows — and zero models of any of the four asserted-absent classes anywhere** (lease, impairment, useful-life review, assets-under-construction). The no-host claims now rest on the registries of every deployment on this host rather than on one, which is the same strengthening P07 applied to its own tenant finding. `P04-F-123`. **The control failing in one identity is the strongest row in the table.** `96548e18` is the never-transacted v18 install where the asset module is **not installed**, so `account.asset` is genuinely absent from its registry and the control **correctly reports absent**. Five identities where the control fires and one where it correctly does not is a **better validation than six where it fires** — it shows the instrument distinguishes a real absence from a failed read, which is the defect this package recorded at `P04-REV-23` and could not otherwise exclude.|**P07's `REV-M-59` corrects its own `F-87` and is the conclusion of this thread.** It had reasoned correctly that no module→model analysis can bound a claim about the absence of a class of model, and concluded **unreachable**. **A class unreachable by the routes a package has is not unreachable in general** — the contribution was not a better answer to its question but **a route that does not need what made the question hard**. And the exchange closed symmetrically: its near-miss check then fired on its own material (`scgl.multi.approve.organization` would have falsified a looser phrasing of `N-25`), so the narrow-boundary rule is now demonstrated on **both** packages rather than argued from one|
| **P04-REV-57** | **P07 found a class its convergence could not reach; the same class is here and larger, and it turns out to be reachable by a different route.** Its *"no tenant ORM model exists anywhere"* is **not a claim about a model but about the absence of a class of model**, so a module→model analysis cannot bound it. This package makes roughly a dozen **"no host"** claims — four unhosted lifecycle stages, the TAS 16 count, no assets-under-construction stage, no lease module, no useful-life review, no asset-register reconciliation — and `P04-F-116`/`-118`/`-121` reach **none** of them. **Bounded instead against the deployment's authoritative model registry**: `ir_model` in `551ab874` holds **944 models**. **Lease, impairment, useful-life review and assets-under-construction: no model of any name.** The whole asset domain is three models — `account.asset`, `account.asset.group`, `account.asset.report.handler`. So the no-host claims are for the first time tested **against a deployment rather than a source tree**, and they hold on a **stronger basis than the one they were published on**. `P04-F-122`, and the route is handed back to P07, whose `N-25` is the same shape and is **not** blocked by the reason it gave.|**The near-misses are the demonstration.** `stock.scrap`, `account.transfer.model` and `stock.valuation.layer.revaluation` all exist in the deployment. **A whole-product phrasing — *"no scrap concept exists"*, *"no revaluation anywhere"* — would have been falsified by them**, and the claims survive only because they were bounded to the asset module. That is `P04-F-121`'s inversion shown on live counter-examples instead of argued: **the narrow boundary was not modesty, it was what made the claim true.** Limits published with it: the registry is **one deployment's**, so this bounds the estate and not the product; and **absence of a model is not absence of a behaviour** — decisive only for claims phrased, as these are, about a missing record type|
| **P04-REV-56** | **Every declared negative re-audited against the four boundaries this session moved — nine survive, one does not.** P07 audited its seventeen by declared boundary and found those bounded to a **path set** and to a **volume** invalidated by changes it had itself made, including one underpinning its tenant finding. Four boundaries moved here: archive **signature set**, archive **path set**, **source scope** (27 installed modules in neither declared root) and **generation basis**. Only the source scope can reach a negative here, because all ten are bounded to **modules**. Test: does any of the 27 undeclared-but-deployed modules declare on the model each negative concerns? **Nine: none.** **One: `equipment_fleet`, on `maintenance.equipment`, source nowhere on this host** — the same module, exposure and six findings already partitioned at `P04-F-118`. `P04-F-121`. **P07's inversion is confirmed here and is worth more than the audit itself.** These ten were written with **narrow, module-scoped** boundaries — *"the asset module"*, *"the loans module"*, *"reporting modules"* — never *"the source tree"* or *"the volume"*, **and they survived a change that invalidated an entire declared root**, where P07's volume-bounded and path-set-bounded negatives did not. **A negative that declares a narrow boundary survives a boundary change; one that declares the widest boundary available at the time does not.** Which inverts the instinct: *"not found anywhere on the volume"* is the bigger claim, and it is the bigger claim a later discovery destroys — **the negative that sounds strongest is the one that rots.**|**The exposure has now been reached from three independent directions and is the same one each time**: `P04-F-116` (stack completeness by model), `P04-F-118` (which findings carry it), and now `P04-F-121` (which negatives carry it) all converge on **one module, one model, six findings and one negative**. That convergence is itself the useful result — it bounds the residue rather than repeating it. Limit unchanged and stated with the claim: *protected* means **no undeclared module declares on that model**; `ir_model_data` sees XML-id declarations only, so a module could override a method and declare nothing. **A floor, not a proof — the nine are safer than they were, not safe**|
| **P04-REV-55** | **The rule published one commit earlier was too noisy to use, and P07's fix for it found one more stale assertion — a settled negative that is now false.** `P04-REV-54` said *enumerate a narrowed finding's citations by identifier and require the qualifier on each*. Run literally it returns **74** here and **162** in P07's package, which named the failure before either of us shipped it: **a check that flags 162 where 3 are real is worse than no check** — this package's own `P04-F-111` warning, turned on its own newest rule. **P07's discriminator: require the qualifier only where the citation *asserts* the narrowed claim.** Applied here — 74 citations, 9 defining blocks, 32 history rows, **18 assertive and live**, of which **17 carry their qualifier**. **The eighteenth is not a wording slip.** `01` §6A.1 still asserted **"No sixth database exists under the stricter method"**. A sixth and seventh snapshot were found, then **`idemo18_uat` — the database two blockers were held open on** — then two more v18 identities. **The sentence had been false for eleven commits**, and it **survived the correction that fixed the identical claim in `P04-REV-27`**: same sentence, same session, two locations, the revision log corrected and the trace file not. That is `P04-F-119` recurring **inside the finding that established it**, one commit later. Corrected in place with the original struck through — a **settled negative asserted from a re-run** is the error `P04-F-105` classifies, and the record should show it was made. `P04-F-120`.|**Final form of the rule, and neither half is sufficient alone: enumerate by identifier — indifferent to phrasing, which is what defeats every grep — but require the qualifier only on citations that ASSERT the claim.** The first half is this package's, the second is P07's. And it is the fourth instance of the pattern P07 states as `REV-M-53`: **a rule's author is systematically the person least able to see its full range, so a written-down method is not a controlled method until someone with a different bias runs it.** Both halves of that were supplied across this exchange and neither session could have supplied both|
| **P04-REV-54** | **`P04-F-100` narrowed `P04-F-99` in two files and left it standing in five.** When a second v18 identity was found with **9 analytic accounts**, where the netting fires **14 of 14**, the latency claim was corrected in `19` — the Layer-1 pack, twice — and in `10`, and **not** in `06` (twice), `09`, `11`, `16` or `20`, each still reading that the dimension is empty *"in the only v18 deployment"*. By then there were **three** v18 identities and that phrase had been false for six commits. `P04-F-119`. **The replacement is stronger than a date fix.** `BD-02` now reads as **breached in both deployments by two different mechanisms** — *absence* where the dimension is empty, *cancellation* where it is populated — so **a design that fixes one does not fix the other**. That was implicit in `P04-F-100` and stated nowhere a reader of `09` would find it. **Why four earlier sweeps missed it:** this package has phrase-grepped for propagation four times (`P04-REV-29`, `P04-REV-31`), and phrase-greps search **wordings** — these five rows say *"the only v18 deployment"*, *"in the only same-generation deployment"*, *"one plan, zero accounts"* and *"latent"*: **four wordings of one superseded figure.**|**Found by P07 asking a question, not by running a check.** Its unit `1c` returned four flags and **zero genuine collisions** — correctly reported as false-positive-prone rather than as four findings, which is this package's own warning applied to this package's own unit — but asking *why* one identifier had definition-shaped rows in five files led to the check that found **fifteen stale restatements** in before/after tables whose *"after"* column was itself superseded. **A clean check result is not a dead end; it is a question about why it is clean.** New rule adopted: **when a finding is narrowed, enumerate its citations by identifier and require the qualifier on each — never grep for the old wording, because the wording is what varies**|
