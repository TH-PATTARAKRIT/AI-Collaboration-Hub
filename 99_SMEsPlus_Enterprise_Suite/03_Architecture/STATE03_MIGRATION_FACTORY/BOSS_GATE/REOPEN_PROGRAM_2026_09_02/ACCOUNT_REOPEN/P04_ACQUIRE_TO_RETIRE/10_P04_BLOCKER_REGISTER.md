...
|----|-----------|--------|-----------|
| **BLK-01** | Which day convention the live asset population actually uses | **MATERIALLY ADVANCED, NOT CLOSED.** A second population is now measured: in `iSMEs`, **683 of 685** assets use daily computation — while **every asset template across the v18 line is on `constant_periods` — 96 of 96, over four snapshots spanning three database identities**, the product default and the convention production does *not* use (`P04-F-81`). Since the two conventions agree annually within 0.05 %, an annual reconciliation cannot detect the mismatch. The blocker names `idemo18_uat`, which was not among the accessible dumps, so it stays open | HOLD — RUNTIME REQUIRED for `idemo18_uat` | One grouped count over the 280 records. Blocks **migration**, not design — and the migration risk is now **concrete** |
