...
1b. **The live configuration is the weakest path — but it is not currently exposed.** All 44 companies in each of the two databases first examined generate on source-document validation, the path with no lock-date check and no catch-up. **Corrected, `34` `W-11` and `W-14`:** zero **deferral** entries have ever been generated (769 **asset** schedules do exist), and **no company in those databases has any lock date set**, so the re-date cannot fire there. The one deployed database that carries locks is the fourth archive, which the parent package wrongly declared unreadable.
