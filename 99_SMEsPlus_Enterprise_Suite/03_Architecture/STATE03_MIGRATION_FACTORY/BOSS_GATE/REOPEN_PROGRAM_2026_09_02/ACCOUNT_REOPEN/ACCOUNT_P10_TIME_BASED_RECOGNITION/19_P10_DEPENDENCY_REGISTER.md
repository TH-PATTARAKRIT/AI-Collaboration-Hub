...
| `D-05` | ~~`P04`~~ → **`P08`** | Ruling that a posting constraint may not silently alter a recognition period | **RE-ROUTED. OPEN — BOSS DECISION.** ~~*adopted programme-wide*~~ — **WITHDRAWN, `38`.** `T0-13` is blocker `P11-B-16`, status **`HOLD — BOSS DECISION REQUIRED`**, and its close condition was **refined** so that where a mutation path has no violation to detect a trace is **mandatory, not alternative**. The wording struck here also carried the superseded unrefined form. Not partially resolved: **open**. See `39`, `40`, `53` `PD-16` |
