...
| `IN-03` | `P04` / `P11` `T0-13` | An accounting fact may be silently mutated at any scope; close condition **refined**: where a mutation path has no violation to detect, a trace is **mandatory, not alternative** | **NOT ADOPTED — `OPEN BLOCKER`, `HOLD — BOSS DECISION REQUIRED`.** ~~ADOPTED~~ withdrawn, `38` §3 | **Misclassified when written.** The verification column recorded an *applicability* argument, not a verification of the peer's status. The peer's status field was never read until `38` |
