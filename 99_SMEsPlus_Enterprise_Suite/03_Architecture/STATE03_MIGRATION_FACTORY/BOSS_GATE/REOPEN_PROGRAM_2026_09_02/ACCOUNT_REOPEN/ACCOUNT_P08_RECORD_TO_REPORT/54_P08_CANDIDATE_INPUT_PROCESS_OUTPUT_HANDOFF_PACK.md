...
| `P08-HO-13` | CANDIDATE HANDOFF | **A boundary actor that destroys ledger facts** — a module deleting settlements, items and entries in unqualified raw SQL and resetting the number sequence | `FACT VERIFIED — P08` | **P11, P06** | `PLAT` | `S16c` mechanism · `D16`/`D19` install state | **installed on all three.** Mechanism read on the **16.0 build only**; the 19.0 build is not on this host. **Capability confirmed; execution NOT evidenced** — three negative probes found no sequence reset and no orphans | **YES** | `58` §5 |
