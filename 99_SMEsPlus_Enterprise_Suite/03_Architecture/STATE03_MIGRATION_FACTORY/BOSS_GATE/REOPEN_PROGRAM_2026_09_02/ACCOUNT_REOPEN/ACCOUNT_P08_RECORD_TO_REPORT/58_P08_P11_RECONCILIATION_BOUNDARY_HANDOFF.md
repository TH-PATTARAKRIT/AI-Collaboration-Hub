...
A module ~~**installed in all three deployed databases**~~ → **installed in ALL FOUR FROZEN RC-05 EXTRACTS** **[`P08-C4`, 2026-09-08 — the deletion module records an **installed** state in `DB-SM`, `DB-BK`, `DB-EV` and `DB-T2`, read from each extract's own module registry table. **Module technical names and registry table names are held in the Layer 2 quarantine and in `RC05_CONFIRMATION_2026_09_08/run/`, not here.** This is a statement about the FOUR FROZEN EXTRACTS, not about the deployed estate, whose size is unproven. Install state is **capability**; whether the path was ever executed is still NOT evidenced.]** deletes, in unqualified raw SQL and in this order: the **settlement table**, then the **journal item table**, then the **journal entry table** — with **no company predicate, no state predicate, a commit after each table**, and a reset of the entry-number sequence to **1**.
...
> **`P08-C1a` — a tool-version precondition travels with the four-input figure.** `DB-T2` is written in a **newer dump-archive format (1.16)** which the **16.x** restore client refuses as an unsupported file-header version. The other three extracts are readable by both **16.15** and **18.6**; **`DB-T2` is readable only by 18.6.** A verifier on the 16.x restore client therefore gets **fail-closed exit 3 on `DB-T2`**, not a fourth zero — **the instrument refuses rather than reporting an unreadable extract as an empty ledger**, which is the behaviour its `C4` control exists to guarantee. **Reproducing the four-input result requires the 18.x restore client or newer.** Executed evidence: `RC05_CONFIRMATION_2026_09_08/run/C7_archive_version_boundary.txt`.
