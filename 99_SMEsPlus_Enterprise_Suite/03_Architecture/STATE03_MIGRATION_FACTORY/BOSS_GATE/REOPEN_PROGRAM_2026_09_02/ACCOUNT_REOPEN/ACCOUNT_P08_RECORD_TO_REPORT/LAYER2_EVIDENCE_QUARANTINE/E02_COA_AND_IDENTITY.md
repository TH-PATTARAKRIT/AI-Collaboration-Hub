...
| `EV-RPT-16` | Correct sibling path in the same module: `l10n_th_reports/models/tax_report_pnd.py` `_rows` uses `report._get_report_query(options, 'strict_range', domain)` → `account_reports/models/account_report.py _get_options_domain` carries `('company_id','in', self.get_report_company_ids(options))` and `_apply_ir_rules`. The VAT handler bypasses both. |
| `EV-RPT-17` | These reports inherit `filter_multi_company = 'tax_units'` from `account/data/account_reports_data.xml:8`, so `options['companies']` resolves to a tax unit or to `_get_branches_with_same_vat(accessible_only=True)` — never to `env.companies`. Populations therefore differ between the on-screen report and the xlsx extract. No country predicate in the VAT path. |
| `EV-CUST-09` | `l10n_th_withholding_tax_report/models/report_withholding_tax.py:154-161` — domain filters `("cert_id.company_partner_id", "=", self.company_id.partner_id.id)`; `cert_id.company_id` never filtered. `l10n_th_withholding_tax_cert/models/withholding_tax_cert.py:128 company_partner_id` (`copy=False`, default `env.company.partner_id`) and `:145 company_id` (required, default `_default_company_id()` → `env.company`, **no `copy=False`**). Isolation rule `l10n_th_withholding_tax_cert/security/account_security.xml` — `['|',('company_id','=',False),('company_id','in',company_ids)]`, i.e. on `company_id` only. |
| `EV-CUST-10` | `l10n_th_withholding_tax/models/account.py update_wt` runs on every tax create and write; searches under record rules (so another company's record is invisible and a duplicate is created) and creates with `company_id` defaulting to `env.company` rather than the source tax's company. |
| `EV-CUST-11` | `l10n_th_withholding_tax_report/report/__init__.py` — sole import commented out while `print_report(report_type="xlsx")` resolves `report_name = "withholding.tax.report.xlsx"`; the defining model file is not imported. Two further unimported duplicate files ship in the module. Static reading only, not executed. |
