...
|---|---|---|---|---|
| ~~`RS-A-01`~~ **WITHDRAWN — `CONTRADICTED`** | ~~No accounting-event model exists.~~ **Retracted after a targeted forensic pass produced database-enforced counterexamples, verified by the author. The pattern — a model-name census on one token — could not match the thing it denied. See `39_P08_ACCOUNTING_EVENT_IDENTITY_FORENSIC.md`.** | anchored model-name census per root, filtered to names containing the event token, deduplicated, with the event-management and framework namespaces excluded (`EV-P-05`) | **0 accounting-event models in 22 of 22 roots.** Residual matches belong to event management, calendar and barcode scanning; **none is an accounting construct**. | **A VERIFIED ABSENCE, scope = the declared 22-root set** |
