...
| `PC-04` | **No period object carrying state, closure, or a link to an entry exists.** A named, dated, owned fiscal-year entity **does** exist in 13 of the 22 roots including the target root — it simply carries none of those things. | **A VERIFIED ABSENCE as re-worded, scope = the declared 22-root set** (`RS-A-02`). *(Re-worded after independent review: the draft's unqualified "no accounting-period entity exists" exceeded its pattern and was contradicted by this file's own `PC-01`/`PC-02`.)* |
