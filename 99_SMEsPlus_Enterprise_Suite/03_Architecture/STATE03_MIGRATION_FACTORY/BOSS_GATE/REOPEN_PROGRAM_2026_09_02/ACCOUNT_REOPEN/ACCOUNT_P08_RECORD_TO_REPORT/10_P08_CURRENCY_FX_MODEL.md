...
| `FX-15` | No tenant dimension exists on the currency, the rate, the item or the company. **Read with §1: this is correct and not a gap for the currency unit and the published rate observation, which are `PLATFORM` scope; it is a gap only for the rate applied to a posting.** | A VERIFIED ABSENCE, scope = the four modules bearing on ledger currency in the target root — the *gap* reading applies only to `SC-FX-04` |
