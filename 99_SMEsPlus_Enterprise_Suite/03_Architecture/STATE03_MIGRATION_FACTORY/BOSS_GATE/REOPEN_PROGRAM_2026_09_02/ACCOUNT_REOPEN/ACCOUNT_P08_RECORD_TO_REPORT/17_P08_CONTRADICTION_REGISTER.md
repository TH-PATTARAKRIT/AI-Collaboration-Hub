...
| `P08-CONTRA-45` | **The Boss-decision total was published as 18 in three files. It is 19.** `P08-BD-19` was defined in `37` and never entered the register in `18` §4 | **CORRECTED AND CLOSED.** All five assertions fixed; the register completed; the total is now **enumerated from the identifiers present**, not asserted. Found by the author running the programme's own orphan-identifier check against a family it had never been run against |
| `P08-CONTRA-46` | **`CQ-P08-12` conflated two units in one total** — 4 named `UNRESOLVED` items and 3 standing integrity limits, published as a single "7" | **CORRECTED AND CLOSED.** Reported as two rows with the units named, and an explicit instruction not to add them |
| `P08-CONTRA-41` | The entry-numbering collision-freedom argument describes a scheme **the measured database does not run** | **RE-SCOPED, not contradicted.** `C NOT YET SEARCHED` for the deployed scheme — `49` §2.2, `P08-U-25` |
