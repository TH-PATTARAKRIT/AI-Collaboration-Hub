...
| C-06 | Bill-line account redirected to the valuation account | `stock_account` | `AccountMoveLine._compute_account_id` | **`R3:stock_account/models/account_move_line.py:13-24` — v19 only** | bill line → valuation account | **file absent in v18** | n/a | n/a | 3,375 bill product lines | **all to expense** (186: 1,062; 72: 966) | **CONTRADICTED FOR SERIES 18 — VERSION-DEPENDENT (v19 mechanism)** | none |
...
| C-08 | Three-way match is advisory, not a control | `purchase_stock` | `qty_received` / `qty_invoiced` | `R1:purchase/models/purchase_order_line.py` | quantities inform, do not block | yes | — | — | 21,102 lines; **1,580 received-not-invoiced ฿30,080,689.78**; 183 invoiced-not-received ฿1,734,752.87 | **divergence persists and is unaccrued** | **CONFIRMED SAME-GENERATION · STRENGTHENED · now quantified** | none |
...
| C-15 | Withholding: repeated full withholding, linear | custom (series-16 wizard) | — | series-16 custom copy | full base per partial payment | **different mechanism here** — `l10n_th` 18.0.2.0 + `account_withholding_tax`, `withholding_tax_cert*`, `account_payment.wt_tax_id` | — | — | 1,183 supplier payments | see AAS-03 Expert C | **DEPLOYMENT-DEPENDENT — different mechanism; series-16 finding does not transfer** | statutory questions → **P07** |
...
| **CONTRADICTED for series 18** | 1 | C-06 |
...
- It does not resolve `C-15`. The series-18 deployment uses a **different** withholding mechanism
  from the series-16 custom wizard P01 analysed; the series-16 finding does not transfer, and the
  series-18 mechanism's statutory correctness is **P07's**, not P01's.
