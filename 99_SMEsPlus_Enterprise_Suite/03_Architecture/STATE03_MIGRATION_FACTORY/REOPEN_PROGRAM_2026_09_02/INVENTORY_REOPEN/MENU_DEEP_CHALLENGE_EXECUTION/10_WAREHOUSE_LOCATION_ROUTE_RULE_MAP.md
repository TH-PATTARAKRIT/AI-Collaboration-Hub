...
Company (tenant-scoped)
 └── Warehouse "คลังหลัก" (code WH)                      MENU-CF-02
      ├── [view] WH                                      (grouping node, holds no stock)
      │    ├── [internal] WH/Stock  "คลังสินค้า"          on-hand stock lives here
      │    │     ├── [internal] Zone A / Shelf 1 ...       optional bins (multi-location switch)
      │    │     └── [internal] "ห้องเย็น" (storage category: cold)
      │    ├── [internal] WH/Input   "รับเข้า (รอตรวจ/รอเก็บ)"  used only in 2/3-step receipt
      │    ├── [internal] WH/Quality "ตรวจคุณภาพ"           used only in 3-step receipt
      │    ├── [internal] WH/Output  "รอจัดส่ง"             used only in 2/3-step delivery
      │    └── [internal] WH/Packing "แพ็กสินค้า"           used only in 3-step delivery
 ├── [vendor]     "ผู้ขาย" (virtual: source of receipts)
 ├── [customer]   "ลูกค้า" (virtual: destination of deliveries)
 ├── [loss]       "สินค้าเสีย/สูญหาย" (virtual: scrap sink)
 ├── [adjustment] "ปรับปรุงยอด" (virtual: count difference counterpart)
 ├── [production] "การผลิต" (virtual: consumption/production counterpart, if MFG)
 └── [transit]    "ระหว่างขนส่ง" (virtual: inter-warehouse / inter-company in-transit)
```
