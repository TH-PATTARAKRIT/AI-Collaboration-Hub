...
> empty are listed (11); the remaining 24 rows are unchanged single anchors (`CHD-13`).
...
| 8 | Lot / Serial | `company + product` | **`company`** — assigned at creation (`L8-10`/`-11`: *"Creation"*); under `CD-04` the product is itself company-anchored; `product` is an identity component, not the ancestor. *(R1 §4.1's sentence to the same effect is voided by `CD-15` and is not relied on — `CHD-06`)* | `CD-04`; `L8-10`; `MTI-12` |
...
| 22 | Count session (`CN-27`) · Inventory adjustment (`CN-28`) | `company + location` | **two objects, one ancestor each:** count session → **`warehouse`** `✎` (a session may span that warehouse's locations — originated here; `L8-13` supplies only *company plus number*); adjustment (the application) → **`location`** | `L8-13`; `MTI-08`; `✎` |
| 23 | Scrap | `company + location` | **`source location`** `✎` (originated; `L8-14` supplies only *company plus number*) | `L8-14`; `✎` |
