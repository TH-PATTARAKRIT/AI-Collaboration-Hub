...
| Handoff to Accounting / Inventory | **To the platform's own Accounting**: an ordinary `BD-ACC-01` emitting handoff — owning domain Billing, occurrence = deduction event, tenant axis = the `PLATFORM` context identifier (the platform is not a tenant of itself, `CHA-10`), company = the operator's legal entity; **`HF-CTX-01`…`-11` apply**. **Tax/accounting treatment and balance-sheet character of prepaid balances: `HOLD / EVIDENCE REQUIRED`, Boss Final Approval** (`27` open item) — the *handoff shape* is specified, the *recognition policy* is not. **No handoff to the customer tenant's Accounting or Inventory** | none (a notice is not a financial fact) | **to the requesting act only**: the decision is a precondition carried as evidence (`HF-CTX-08`), not a business fact | none |
