...
| 3 | `SA15` status line and §4 | *4 traversable (E2E-02, -11, -12, -15)* | **3 traversable (E2E-02, -11, -12)**; 13 with named break; 2 not traversable | rows 1–2 | consequential |
| 4 | `SA15` §4 check line | *"4 + 7 + 7 = 18"* over a table enumerating 13 identifiers with printed counts 4/7/2 | **3 + 13 + 2 = 18**, all 18 identifiers enumerated | `JCP3-F-11` (CORR3, `MINOR`, never applied) | **a self-check that was false as written** |
...
| 10 | `SA17` §2 rows 16–17 `E2E-11`/`-12` | *"—"* (no inputs) | `JT-05` Boss election; return basis conflict `PENDING` | `SA_CORR3_06` rows 8–9; `SA_CORR5_10` §3 | **a `TRAVERSABLE` grade carrying an undecided accounting value** — the route traverses; the value does not |
...
| 15 | `SA17` §3, §4, §6 | — | three controls added (`MTI-33`, `CF-I-03R`, `MTI-29`/`TRG-02`); `ND-09`, `-11` carried in; `ND-12`, `-13` added; bounded risks gain element 15 and the COGS residual | CORR2/CORR5 | completion |

**15 corrections; 2 artefacts; 0 historical bytes changed.**
...
|---|---|---|
| `SA15` traversable / named break / not traversable | 4 / 7 (table) — 12 (header) / 2 | **3 / 13 / 2** |
| `SA17` scenarios graded `TRAVERSABLE` with an undecided accounting value | 0 declared | **2 declared** (`E2E-11`, `-12`) — the grade stays, the value's owner is named |
...
| Runtime-obligation registers attached | 0 | **7** |
...
> ## `CP-SA-C5-60 — PRE-TEST HANDOFF DATA CORRECTED`
> **15 corrections across 2 controlled versions · `C4-07-F-03` closed · `JCP3-F-11` applied · 5
> propagation failures repaired at the row · 3 prohibitions and the dependency order now travel with
> the baseline · 0 historical bytes overwritten · `SA15`: 3 / 13 / 2.**
