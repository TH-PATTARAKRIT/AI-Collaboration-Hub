...
**BINV-17 added, Round 6**), fifteen business rules, **twelve** conceptual entities (Fiscal Year
and the Effective-Date/Recorded-At split added Round 2; Reported Retained Earnings added Round
3; Reported Equity (§1f) and viewpoint parameterization (§1g, corrected Round 6) added Round 4;
Fiscal Year boundary versioning (§1h, corrected Round 6) added Round 5; **viewpoint-aware
Fiscal Year definition (§1i) and post-reliance membership reconciliation (§1j) added Round 6**,
B07), **twelve** mathematical design principles (MP-09 rebuilt with a two-temporal-mode model,
MP-11 new, Round 2; MP-11 rewritten Round 3; MP-11 cross-reference corrected and MP-12 added
Round 4; MP-09 renamed and split, MP-12 Proof G rebuilt into G1-G4, Round 5; **MP-09's boundary
lookup and MP-12 Proofs D/G4 corrected for viewpoint consistency, Round 6**), **sixteen**
control objectives (CO-14/CO-15 added Round 2; CO-16 added Round 3; CO-14 scope extended Round
4, Round 5, and again Round 6; CO-15 extended Round 5 and again Round 6), **sixteen** migration
requirements (MG-C14 added Round 2; MG-C15 added Round 4; MG-C11 corrected and MG-C16 added
Round 5), **twenty-two** exception scenarios (scenario 19 added Round 3; scenario 20 added Round
4; scenario 21 added Round 5, corrected Round 6; **scenario 22 added Round 6**), nine advancement
items with measurable criteria, **thirteen** formally compared design-option decisions
(DT-08/DT-09 added Round 2; DT-10 added Round 3; DT-11 added Round 4; DT-12 added Round 5;
**DT-13 added Round 6**). Full index:
...
authorization tier). Full record: [CORR_B5_TRIAL_BALANCE_FISCAL_CALENDAR_CORRECTIVE_ROUND.md](CORR_B5_TRIAL_BALANCE_FISCAL_CALENDAR_CORRECTIVE_ROUND.md).

**Round 6 — two further findings, both corrected, BOTH introduced by Round 5's own fix:**
`M-AUD-13` (CRITICAL — B07 §1g's Round-4 claim that the Elapsed test "never takes a viewpoint
parameter" directly contradicted §1h's own Round-5 Known/Current calendar model, never revised
to match — closed by correcting §1g in place and formalizing `FiscalYearDefinition_Known/Current`
and `Elapsed_Known/Current` at new §1i, propagated into B08 MP-09/MP-12 Proofs D/G4);
`M-AUD-14` (CRITICAL — §1h's own Round-5 post-reliance change model permitted a new boundary
version to coexist indefinitely with old, un-reclassified Entry membership, with no defined
Current-viewpoint reporting behavior for that state — closed by adopting Option A (Prospective-
Only Change After Reliance), refined with a new, dedicated, atomic `FiscalYearMembershipRestated`
mechanism (B04, B07 §1j) that moves the boundary and reclassifies affected Entries' membership
in one indivisible action; B07's Fiscal Year cardinality/identity statement corrected to be
viewpoint/version-safe). This round's own regression (B23) did not surface a further gap
...
Round 6 corrective commit `(recorded after commit)` pushed to
...
returned by Round 6 re-audit → this status, true as of `(recorded after commit)`, specifically
for Round 6's corrections.
...
Per explicit instruction across every round, `ERPPLUS-100`'s Assignee (`UNASSIGNED`) and Due
