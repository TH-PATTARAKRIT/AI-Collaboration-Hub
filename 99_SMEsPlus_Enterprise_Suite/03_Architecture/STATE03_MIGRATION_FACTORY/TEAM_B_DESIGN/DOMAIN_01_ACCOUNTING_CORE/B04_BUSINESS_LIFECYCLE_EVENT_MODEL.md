...
| **Corrected (Round 6)** | **CORR-B6-02 (2026-08-30)** — ChatGPT's Round 6 re-audit (`b0ce666dad72909411a49690d0f642313d94dd13`, finding `M-AUD-14`) found `FiscalYearBoundaryChanged`'s Round-5 scope left it ambiguous whether it could reach backward over already-relied-upon dates while leaving Entry membership frozen — the exact hybrid state the audit flagged. `FiscalYearBoundaryChanged`'s row corrected below to be constitutionally prospective-only past reliance; new `FiscalYearMembershipRestated` event added for the genuinely rare, atomic, post-reliance correction case (B07 §1j). See [CORR_B6_FISCAL_CALENDAR_VIEWPOINT_MEMBERSHIP_CORRECTIVE_ROUND.md](CORR_B6_FISCAL_CALENDAR_VIEWPOINT_MEMBERSHIP_CORRECTIVE_ROUND.md). |
...
| `FiscalYearBoundaryChanged` *(new, added at CORR-B5-05; scope corrected at CORR-B6-02)* | ~~An authorized, CO-15-tier action changes a Fiscal Year's Start/End boundary AFTER that boundary has already governed a COMMITTED Entry, elapsed, or been referenced by an issued/consumed report (B07 §1h) — pre-reliance corrections require no such event, since nothing has yet depended on the old value~~ **CORRECTED AT CORR-B6-02 (`M-AUD-14`):** an authorized, CO-15-tier action changes a Fiscal Year's Start/End boundary for a Fiscal Year with **no existing reliance** (pre-reliance correction, or a genuinely future, not-yet-begun Fiscal Year — [B22](B22_CORR_B5_TRIAL_BALANCE_AND_FISCAL_CALENDAR_REGRESSION.md) Test 14) — its Version Effective Date may never fall earlier than the point reliance began. **This event can no longer reach backward over a Fiscal Year that already has a COMMITTED Entry, is Elapsed, or has been referenced by an issued/consumed report — that case is `FiscalYearMembershipRestated` (below), a different event, never this one.** | ...the boundary change itself is later found mistaken while still pre-reliance (a further, chained `FiscalYearBoundaryChanged`, same authorization tier — never a silent revert) |
| `FiscalYearMembershipRestated` *(new, added at CORR-B6-02)* | An authorized, CO-15-tier-or-stricter action atomically (a) changes a Fiscal Year's Start/End boundary for Current-viewpoint purposes AFTER it already has reliance, AND (b) reclassifies every affected Entry's Current-viewpoint Fiscal-Year membership, in the SAME action — never two independently-timed actions (B07 §1j). Records scope, old/new boundary versions for every Fiscal Year touched, reason, actor, Recorded At, Effective Date, and every affected reporting period. The Known view for any T before this event's own Recorded At is structurally unaffected (B07 §1i) | ...the reclassification itself is later found mistaken (a further, chained `FiscalYearMembershipRestated`, same authorization tier — never a silent revert, never a partial/non-atomic undo) |
...
CORR-B4-03/CORR-B5-05/CORR-B6-02 — see header. §3a is new at Round 2; §3b/§3c new at Round 3;
the event table gained `Restated` and `FiscalYearClosed`, lost `CarriedForward` (removed
deliberately, explained above, not silently deleted); `FiscalYearClosed`'s own description was
corrected at Round 3 (no posted Entry) and again at Round 4 (no reporting-inclusion effect,
`M-AUD-09`). `FiscalYearBoundaryChanged` is new at Round 5, protecting the calendar facts
`FiscalYearClosed` and the Elapsed test (B07 §1e) both depend on from silent retroactive editing
(`M-AUD-12`); its own scope was corrected at Round 6 to be constitutionally prospective-only
past reliance, and a new, dedicated `FiscalYearMembershipRestated` event was added for the
genuinely rare post-reliance correction case, closing the hybrid-state gap `M-AUD-14` found
(B07 §1j).)*
