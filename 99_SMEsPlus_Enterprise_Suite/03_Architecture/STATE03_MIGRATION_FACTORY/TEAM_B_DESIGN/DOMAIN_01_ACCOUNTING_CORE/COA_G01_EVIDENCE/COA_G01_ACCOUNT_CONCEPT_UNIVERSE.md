...
| Tax Branch (sub-Company entity) | (organizational, not an account type) | Mandatory where Thai statutory filing applies | Mandatory — Company Context alone (2-level) is insufficient for Thai filing | VERIFIED FACT (local, not yet on GitHub — see C-01) | Local finding S5 |
| WHT recognition timing (at payment, not invoice) | Affects Payable/Receivable posting behavior | n/a | Company-scoped posting rule | VERIFIED FACT (local, not yet on GitHub — see C-01) | Local finding S2 |
