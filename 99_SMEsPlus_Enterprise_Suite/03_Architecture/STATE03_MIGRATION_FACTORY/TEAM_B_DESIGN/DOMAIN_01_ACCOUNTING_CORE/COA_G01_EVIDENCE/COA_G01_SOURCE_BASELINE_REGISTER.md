...
|---|---|---|---|
| A | Team A Deep Research / Accounting Core evidence | VERIFIED FACT (partial) | Local `STATE03` findings S1–S11 (frozen 2026-08-23, file:line cited against source), local Thai toolchain findings T1–T9. **Not yet committed to GitHub** — see `COA_G01_SOURCE_CONFLICT_REGISTER.md` item C-01. |
...
| D | Boss-approved `Odoo18` workbook tab | VERIFIED FACT (extraction only; raw workbook file not present in local `ACCOUNT` folder) | 389 data rows (index 0–388), 5 columns (`id`, `name`, `reconcile`, `code`, `account_type`), 14 `account_type` labels used. Evidence commit `ae2b0719081ef9497f08e3b3e1ea8329d053cf83` (verified: "evidence(state03): inventory Odoo18 COA tab rows and columns"). The source `.xlsx` itself was not located during the local-folder search performed for this session — only vendor generic import templates exist locally (e.g. `account/static/xls/coa_import_template.xlsx`), which are **not** the same artifact. This is not treated as EVIDENCE_MISSING at the Gate level because the extraction is already committed evidence, but the absence of the primary workbook file itself is logged as a traceability gap. |
...
| G | Existing Boss rulings, PMO evidence, ChatGPT audit evidence | VERIFIED FACT | 23 `BOSS_GATE`/`CHATGPT_AUDIT`/`PMO_VERIFICATION` documents read in full on the `SMEsPlus` branch (see `COA_G01_EVIDENCE_MANIFEST.md` for the full list). |
