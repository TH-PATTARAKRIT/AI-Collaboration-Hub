...
| **`AAS-V-03`** | AAS+ | any Cross-Context Report Grant carrying valuation content while the COGS gap stands | **YES** for the register; the gap's content is Boss's | **YES — `F6` (`MTI-D-04`) and `F1` (`JT-05`)** | No | No | **No** | **AAS+ after Boss rules `F6`** — and under the recommended *"no grant in v1"* the veto's subject **ceases to exist**, which is the cheapest discharge available |
| **`CF-V-02`** | AAS+ | citing `CF-I-06` as reducing `RC-F-03`, or `CF-I-08` as reducing `RC-F-07` | **YES** — both are correctly cited as a prohibition and a scope rule throughout this package (sweep at `SA_FINAL_08` §5) | **YES — `F6` (`MTI-D-04`, `RC-D-04`) and `F6` (`RC-D-03`)** | No | No | **No** | **AAS+ after Boss rules `F6`** |
| **`AAS-V-02`** | AAS+ / Boss | implementation start before `MTI-D-01`, `-D-02`, `-D-03` are ruled | n/a — a precondition veto | No — **its condition is satisfied**: all three are `BOSS RULED` (2026-09-04) | No | No | **YES — condition satisfied** | **AAS+ discharge act, ratified by Boss.** Requested as a Boss **act**, not a decision (`SA_FINAL_03` §4). **Implementation start remains barred by `RC-V-01` regardless** |
...
| Held open by a **Boss decision** | 2 — both in family `F6` |
| Held open only by a **ratification act** | 1 |

> **`FG-F-05`. Two of the six vetoes — a third of the standing veto set — are discharged by one Boss
> ruling, `MTI-D-04`.** Under the SMEs Core recommendation (*no cross-company grant in v1*)
> `AAS-V-03`'s subject ceases to exist and `CF-V-02`'s first limb closes with `RC-F-03`. **No prior
> round has stated the veto set's dependency on a single election**, and it is the highest-leverage
> item on the Boss list by that measure.
