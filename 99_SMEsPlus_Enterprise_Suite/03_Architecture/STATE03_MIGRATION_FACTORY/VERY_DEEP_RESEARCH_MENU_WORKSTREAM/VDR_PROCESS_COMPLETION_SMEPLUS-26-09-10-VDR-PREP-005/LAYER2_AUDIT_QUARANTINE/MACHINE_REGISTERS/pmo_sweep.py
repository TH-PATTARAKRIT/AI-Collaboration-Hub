...
IDPAT=re.compile(r'\b((?:MM|CD|FT|FN|OD|XM|SS|HA|EB|FO|RR|RC|DR|P3|C3|O3|R3|PW3|R4|CF4|OD4|PR4|LESA4|PMO4)-F-\d{2}|(?:FE|EV-DB|EV-CH|EV-SRC|EV-RT|EV-A|PMO4|P4-R)-\d{1,2}[A-Z]?|GAP-INV-\d{2}[A-Z]?|CRITICAL-GAP-\d{2}|BOSS-DEC-\d{2}|CORR-F-\d{2}|SR-\d{2}|GOV-\d{2}|INV-M\d{2}|INV-F-\d{2}|JT-\d{2}|IV-\d{2}|P-\d{2}|R4-F-\d{2}|GAP-MD-\d{2}|LI-INV-[A-Z]+-\d{4}|CORR-\d{3}[A-Z]?)\b')
...
DEFLINE=re.compile(r'^\s*(?:###+\s*(?:\*\*)?`?|\|\s*(?:\*\*)?`?)((?:MM|CD|FT|FN|OD|XM|SS|HA|EB|FO|RR|RC|DR|P3|C3|O3|R3|PW3|R4|CF4|OD4|PR4|LESA4|PMO4)-F-\d{2}|(?:FE|EV-DB|EV-CH|EV-SRC|EV-RT|EV-A|PMO4|P4-R)-\d{1,2}[A-Z]?|GAP-INV-\d{2}[A-Z]?|CRITICAL-GAP-\d{2}|BOSS-DEC-\d{2}|CORR-F-\d{2}|SR-\d{2}|GOV-\d{2}|INV-M\d{2}|INV-F-\d{2}|JT-\d{2}|GAP-MD-\d{2}|IV-\d{2}|P-\d{2}|R4-F-\d{2})\b')
for p in md:
...
# Dotted prefixes are IDENTIFIER prefixes and must be followed by an identifier character.
# Matching 'stock.' as a bare substring also matches the English sentence '...real stock.' —
# the same false-positive class as matching 'quant' inside 'quantity'.
DOTTED=['stock.','product.','mrp.','uom.','ir.ui','ir.actions','ir.model','ir.cron','ir.rule',
        'res.config','res.groups','res.partner','account.move']
SUB=['openerp','_action_','__manifest__','.py','l10n_','scgl_','smesplus_','odoo']
WORD=['quant','quants','orderpoint','orderpoints','picking','pickings','sudo','addons','Odoo','ODOO']
tokrep=[]
for p in md:
    txt=open(p,encoding='utf-8',errors='replace').read()
    low=txt.lower()
    n=sum(low.count(t.lower()) for t in SUB)
    n+=sum(len(re.findall(r'\b'+re.escape(t)+r'\b',txt)) for t in WORD)
    n+=sum(len(re.findall(re.escape(t)+r'[a-z_]',low)) for t in DOTTED)
    hits=[t for t in SUB if t.lower() in low]
    hits+=[t for t in WORD if re.search(r'\b'+re.escape(t)+r'\b',txt)]
    hits+=[t for t in DOTTED if re.search(re.escape(t)+r'[a-z_]',low)]
...
print(json.dumps({
 ...
 'CHECK_4_TOKEN':{'files_scanned':len(tokrep),
   'token_classes':{'substring':len(SUB),'word_boundary':len(WORD),'dotted_prefix':len(DOTTED)},
