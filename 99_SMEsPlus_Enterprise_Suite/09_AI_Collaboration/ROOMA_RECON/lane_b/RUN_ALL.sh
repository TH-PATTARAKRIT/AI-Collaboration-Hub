...
# RT-LANEB-016 (26 Sep): the gate must ask the SERVER as the observer, live.
# The old grep read the grant script's own output (odoo-shell view); the HTTP
# workers disagreed with it for 6.5 h. ops/laneb_preflight_unlink.py exits 0
# only when has_access(records,'unlink') is True right now.
python3 "$W/ops/laneb_preflight_unlink.py" 2>&1 | tee -a "$R"
if grep -q "UNLINK_RIGHT: LIVE" "$R" 2>/dev/null; then
