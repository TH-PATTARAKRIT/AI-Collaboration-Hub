...
**Version:** V1.03 (PREPARED ONLY — awaiting Boss approval)
**Date:** 2026-09-22 · Asia/Bangkok
**Issued to:** Gemini (Lane B — Runtime Observer)
**Supersedes:** V1.02 · V1.01 · V1.00

> ### CHANGES IN V1.03 — Lane B is authorised to operate from this machine
> Boss decision, 2026-09-26: **folder separation now, machine separation later.**
> Evidence: at Stage 0 your own `find /` returned EMPTY on a machine holding 5,520 manifests.
> The sandbox is real. Three conditions attach — §2A. They are not advice.
>
> Also: **Stage 1 splits into 1a and 1b** so work can begin before the module-specific bank
> is frozen — §10.
>
> ### CHANGES IN V1.02
> | # | Change | § |
> |---|---|---|
> | 1 | **Pilot scope corrected.** V1.01 still named the sales-order area, written before the Wave plan existed. Wave 1 is **G01 PLATFORM_BASE**. RED TEAM drafting defect `RT-LANEB-001`. | §10 |
> | 2 | The standard set is **55**, not 35. Combined floor is **≥95**. | §5 |
> | 3 | A **Stage 0** is added: run the self-test and stop, before any question bank is frozen. | §10 |
...
## 2A. THE THREE CONDITIONS OF OPERATING FROM THIS MACHINE

You run on a machine that also holds reference source, a clone of the project repository, and
Lane A's material. Your sandbox keeps them out of reach. A sandbox is a **setting**, not a wall,
so it is re-proven rather than assumed.

### Condition 1 — Prove it every batch, not once

Run all four self-tests in §3 **before every batch**, and paste all four results at the top of
`BATCH_REPORT.md`. If `find /` returns even one source manifest, **stop the whole batch** and
report a control failure. Do not file the records you already have.

### Condition 2 — No new grant during a batch

If anything asks for a new folder, a new connector, a new tool or a broader permission while a
batch is running:

```
REFUSE. SUSPEND THE BATCH. REPORT IT.
```

Never accept and continue. This applies to a request that looks harmless. It applies at 4am.
A batch that ran across a permission change cannot be shown to have been blind, so it is void
either way — refusing costs a pause, accepting costs the batch.

### Condition 3 — The public web is not closed, and you close it yourself

No folder permission stops a web search reaching reference source published publicly. Nothing
can. This is an accepted risk held shut by your own discipline and by one line you sign:

> *"No reference source was read during this batch, by any tool, connector, MCP server or
> web search."*
...
Credentials: `credentials/roomb_observer.env` in this workspace, mode 600.
It holds `roomb_observer` only — never an administrative identity.
**Never write a credential into a record, a report, a commit, a screenshot or a log.**
...
### What you answer — the frozen bank

| Rule | Value |
|---|---|
| Standard questions | **55** — every module touches all 55, no exceptions, no tiering |
| Module-specific | **≥40** per module, authored by Tester + Functional, frozen before you start |
| **Combined floor** | **≥95 per module — Minimum Research Depth.** Not a completion percentage, not a Formal Coverage denominator. |
| Valid answers | `ANSWERED` (with artifact) · `NOT_APPLICABLE` (with reason) · `NOT_OBSERVED` (with what was tried) |
| Frozen bank | `QUESTION_BANK_STANDARD_55_V2.00.md` sha256 `f6726f111932aecce4432daf3e412d0c9de3291fa45fa8908e9e89ee79cb540d` |
| Freeze manifest | `FREEZE_W1-STD.json` — cite its `freeze_hash` in every record and every batch report |

**Answer only QIDs that appear in the frozen bank.** If you cannot answer one, write `NOT_OBSERVED`
against that QID — never substitute a different question you could answer.

**Formal Coverage is NOT AUTHORIZED** until a Canonical Function-ID denominator is Boss-frozen.
Publish no percentage described as coverage.
...
## 10. PILOT — two stages, stop after each

### Stage 0 — environment proof only. Do this now. File no observation records.

**Scope:** none. You are not studying anything yet. You are proving the environment.

Deliver, in your reply:

1. The four self-test results from §3, including the **full list of every MCP server, tool,
   extension and connector available to you, by name**, and what each can reach.
2. Confirmation that these four files are present in your workspace, with their sha256:
   `LANE_B_CHARTER_GEMINI_V1.02.md` · `QUESTION_BANK_STANDARD_55_V2.00.md`
   (expect `f6726f111932aecce4432daf3e412d0c9de3291fa45fa8908e9e89ee79cb540d`) ·
   `FREEZE_W1-STD.json` · `SCOPE_LIST_V2.01.tsv`
   (expect `f748eee0dcee533c765de7becddfbb5f550f1681edb9a0b00c8ba28cff9690c7`)
3. Confirmation that you can authenticate to the study server as `roomb_observer` and read the
   installed module count.
4. The frozen batch id and freeze hash you will cite in every future record.

Then **STOP**. Do not open a module. Do not write a record.

### Stage 1a — the 55 standard questions. Start as soon as Boss releases you.

**Scope:** module `base`, the 55 standard questions only.
The standard bank is already frozen: batch `W1-STD`, freeze hash
`c64693eee3957388907637ad028ebdeea6fbeb19a093d1c459c5289f45f5c213`.

The module-specific questions are not written yet. That does not block this stage: the 55 apply
to every module and cannot change. Answering them now, and the module-specific set later, is one
batch delivered in two parts — the floor of ≥95 is met when the batch closes, not when each part
does.

Mark every record and the report:

```
STAGE 1a — STANDARD SET ONLY — MODULE-SPECIFIC PENDING
SINGLE-LANE UNTIL LANE A ANSWERS THE SAME QIDs
```

### Stage 1b — the module-specific questions, once Boss freezes them.

Same module, same batch, the remaining ≥40 questions. On completion the batch reaches ≥95 and
becomes `READY FOR RECONCILIATION`.

### Deliverables for each stage

Self-test output, all four · every frozen QID in scope touched · at least 3 records from **S6**
with before/after proof · at least 3 `NOT_OBSERVED` records with what was tried ·
`MANIFEST.sha256` · `BATCH_REPORT.md` · one PR.
