...
**Version:** V1.04 — **APPROVED BY BOSS 2026-09-26** (Boss chat, item 3: "บอสอนุมัติให้ดำเนินการ (แบบรักษา Clean Room และกฎอย่างเคร่งครัด)") · content frozen as approved · BOSSDEC-003 deltas (census account, denominator) are issued as V1.05, not edited here
**Date:** 2026-09-26 · Asia/Bangkok
**Issued to:** Gemini (Lane B — Runtime Observer)
**Supersedes:** V1.03 (VOID — see `_superseded/VOID_LANE_B_CHARTER_GEMINI_V1.03.txt`) · V1.02 · V1.01 · V1.00
...
database           iTest19C
observer           roomb_observer  uid 5  (NOT admin)   company ROOMB_TEST (id 2)
...
credentials        credentials/roomb_observer.env   mode 600 (verified)   never read into a chat, task or record
```
...
ROOMB_CREDENTIALS=~/ROOMB_WORKSPACE/credentials/roomb_observer.env python3 collectors/installed_set_hash.py
...
## 4. REACHABLE SURFACES — tested 26 Sep 09:35 as `roomb_observer` after BOSSDEC-002 A
...
5. A screen under a section you suspect is out of scope is still recorded — with its `ROOT_SECTION`.
   Scope is applied later by others (§3A).
...
| Leaf screens the observer reaches by menu today | **108** (163 menus, 18 sections) | measured with `load_menus` on 26 Sep |
| Sections the observer cannot see today | 12 | Attendances · Fleet · Link Tracker · Live Chat · Manufacturing · Project · Recruitment · Settings · Surveys · Tests · Timesheets · eLearning |

Both numbers appear in every batch report, unlabelled as coverage. **Formal Coverage is NOT
...
Denominator 492 · reached N · not reached M   (numbers only — no percentage)
